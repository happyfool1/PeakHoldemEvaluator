	...
	******************************************************************************/
	...
	static String showdownString() {
		...
		st += new StringBuilder().append("The winner was seat ").append(String.valueOf(EvalData.winnerSeat))
			...
			st += new StringBuilder().append("There was a tie ").append(String.valueOf(players)).append(" winners\r\n")
				.toString();
		}

				st += new StringBuilder().append("There were ").append(String.valueOf(players)).append(" winners\r\n")
				.toString();

		for (int i = 0; i < PLAYERS; i++) {
			if (!EvalData.playerFolded[i] && EvalData.showdownHand[i]!= MADE_NONE) {
