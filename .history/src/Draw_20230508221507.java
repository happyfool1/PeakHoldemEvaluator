...
public class Draw implements Constants {
	...
	*******************************************************************************/
	boolean draw(int type) {
		...
		case DRAW_BOARD_PAIR -> result = isBoardPair();
		case DRAW_PAIRED_WITH_BOARD -> result = isPairedWithBoard();
	 ...
	 ***************************************************************************************/
	boolean isFlushOESD() {
		if (EvalData.drawArray[DRAW_OESD] && EvalData.drawArray[DRAW_FLUSH]) {
			return true;
		}
		return false;
	}
