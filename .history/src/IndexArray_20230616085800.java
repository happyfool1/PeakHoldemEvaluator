...
public class IndexArray implements Constants {
            ...
            String[] madeColNames) {
               for (int i = 0; i < drawArray.length; i++) {
                for (int j = 0; j < drawArray[i].length; j++) {
                    this.drawArray[i][j] = drawArray[i][j];
                }
            }
            for (int i = 0; i < madeArray.length; i++) {
                for (int j = 0; j < madeArray[i].length; j++) {
                    this.madeArray[i][j] = madeArray[i][j];
                }
            }
            for (int i = 0; i < showdownArray.length; i++) {
                for (int j = 0; j < showdownArray[i].length; j++) {
                    this.showdownArray[i][j] = showdownArray[i][j];
                }
            }
            for (int i = 0; i < rowNames.length; i++) {
                this.rowNames[i] = rowNames[i];
            }
            for (int i = 0; i < drawColNames.length; i++) {
                this.drawColNames[i] = drawColNames[i];
            }
            for (int i = 0; i < madeColNames.length; i++) {
                this.madeColNames[i] = madeColNames[i];
            }


       
        System.out.println("zzz " +  drawArray[6][0] + " " +  this.drawArray[6][0]     );
    ...
    private void copyArrays() {
        ...
        for (int i = 0; i < this.showdownArray.length; i++) {
            for (int j = 0; j < this.showdownArray[0].length; j++) {
                this.showdownSumOfAllValuesPer += this.showdownArray[i][j];
            }
        }
    ...
    ***************************************************************************************** */
    private String stringStrategy() {
        System.out.println("//xxx " + bestDrawRowsPer[0]);
        ...
        st += "The worst Draw rows were " + allArrayRowNames[worstDrawRows[0]] + ", "
                + allArrayRowNames[worstDrawRows[1]]
                + ", " + allArrayRowNames[worstDrawRows[2]] + "\r\n";
        st += "The best Made rows were " + allArrayRowNames[bestMadeRows[0]] + ", " + allArrayRowNames[bestMadeRows[1]]
                + ", " + allArrayRowNames[bestMadeRows[2]] + "\r\n";
        st += "The worst Made rows were " + allArrayRowNames[worstMadeRows[0]] + ", "
                + allArrayRowNames[worstMadeRows[1]]
                + ", " + allArrayRowNames[worstMadeRows[2]] + "\r\n";
        st += "The best Showdownw rows were " + allArrayRowNames[bestShowdownwRows[0]] + ", "
                + allArrayRowNames[bestShowdownwRows[1]]
                + ", " + allArrayRowNames[bestShowdownwRows[2]] + "\r\n";
        st += "The worst Showdown rows were " + allArrayRowNames[worstShowdownRows[0]] + ", "
                + allArrayRowNames[worstShowdownRows[1]]
                + ", " + allArrayRowNames[worstShowdownRows[2]] + "\r\n";

        st += "The best Draw cols were " + drawArrayColNames[bestDrawCols[0]] + ", "
                + drawArrayColNames[bestDrawCols[1]]
                + ", " + drawArrayColNames[bestDrawCols[2]] + "\r\n";
        st += "The worst Draw cols were " + drawArrayColNames[worstDrawCols[0]] + ", "
                + drawArrayColNames[worstDrawCols[1]]
                + ", " + drawArrayColNames[worstDrawCols[2]] + "\r\n";
        st += "The bes Made cols were " + madeArrayColNames[bestMadeCols[0]] + ", " + madeArrayColNames[bestMadeCols[1]]
                + ", " + madeArrayColNames[bestMadeCols[2]] + "\r\n";
        st += "The worst Made cols were " + madeArrayColNames[worstMadeCols[0]] + ", "
                + madeArrayColNames[worstMadeCols[1]]
                + ", " + madeArrayColNames[worstMadeCols[2]] + "\r\n";
        st += "The best Showdownw cols were " + madeArrayColNames[bestShowdownwCols[0]] + ", "
                + madeArrayColNames[bestShowdownwCols[1]]
                + ", " + madeArrayColNames[bestShowdownwCols[2]] + "\r\n";
        st += "The worst Showdown cols were " + madeArrayColNames[worstShowdownCols[0]]
        ...
        st += "The best 5 draws were " + Format.formatPer(bestDraw5Per[0])
                + " " + allArrayRowNames[bestDraw5Row[0]] + ", " + drawArrayColNames[bestDraw5Col[0]] + "\r\n";
...
/*- ************************************************************************************** 
 Please note that this method is just a simple example, which may not reflect the exact score you want to calculate. In real poker game analysis, you might want to weight different types of hands differently, consider more factors (like the number of players, the position, etc.), or use more sophisticated statistical methods. You would need to adjust the method accordingly based on your specific requirements.

The method works by simply adding up the proportions of each type of made hand and draw for the given HML index, then converting this sum to a percentage. This gives a simple measure of how "good" the hand is, with a higher score indicating a better hand. However, this measure is quite simplistic, and may not accurately reflect the actual quality of the hand in a real poker game.
*/
