...
public class Classification implements Constants {
	...
	***************************************************************************************************/
	 ...
	 *  a rank and a suit. Let's represent ranks as integers from 2 (for 2) to 14 
	 ...
	 * This will return a unique integer from 1 to 9 for each possible combination. The logic for the computation is that we subtract 1 from the suitType and multiply by 3 (since we have three connectivity types), and then we add the connectivityType. This ensures a unique value for each combination.
	 * 
	 *******************************************************************************/
	public static int flopTexture(Card[] cards) {
		...
		// Sort cards by value
		Arrays.sort(cards, Comparator.comparing(card -> card.value));
		...
		return (suitType - 1) * 3 + connectivityType;
	}
