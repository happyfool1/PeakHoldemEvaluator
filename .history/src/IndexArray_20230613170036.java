...
public class IndexArray implements Constants {
    ...
    ***************************************************************************************** */
     ...
     * have thr roe or column totals.
     *  
     ***************************************************************************************** */
    void createIndexArrayRowAndColumnList() {
        for (int row = 0; row < drawTotalRow.length; row++) {
            drawTotalRowEntries.add(new PairEntry(drawTotalRow[row], row));
        }

        drawEntries.sort(Comparator.comparing(PairEntry::getPercentage));
        for (int row = 0; row < madeArrayPer.length; row++) {
            for (int column = 0; column < madeArrayPer[row].length; column++) {
                madeEntries.add(new PairEntry(madeArrayPer[row][column], row, column));
            }
        }
        madeEntries.sort(Comparator.comparing(PairEntry::getPercentage));
        for (int row = 0; row < showdownArrayPer.length; row++) {
            for (int column = 0; column < showdownArrayPer[row].length; column++) {
                showdownEntries.add(new PairEntry(showdownArrayPer[row][column], row, column));
            }
        }
        showdownEntries.sort(Comparator.comparing(PairEntry::getPercentage));
    ...
    ***************************************************************************************** */
    ...
    // Sorted list for each array and reversed
    List<PairEntry> drawTotalRowEntries = new ArrayList<>();
    List<PairEntry> madeTotalRowEntries = new ArrayList<>();
    List<PairEntry> showdownTotalRowEntries = new ArrayList<>();
    List<PairEntry> drawTotalColEntries = new ArrayList<>();
    List<PairEntry> madeTotalColEntries = new ArrayList<>();
    List<PairEntry> showdownTotalColEntries = new ArrayList<>();
