	...
	static void updateSummary() {
		...
		for (int i = 0; i < HML_FLOP_SIZE; i++) {
			for (int j = 0; j < MADE_SIZE; j++) {
				EvalDataMany.hhmlMadeRowTotal[i] += EvalDataMany.hmlCountFlopmlMadeFlop[i][j];
				EvalDataMany.hmlMadeSumOfAllValues += EvalDataMany.hhmlMadeFlop[i][j];
			}
		}
		for (int j = 0; j < MADE_SIZE; j++) {
			for (int i = 0; i < HML_FLOP_SIZE; i++) {
				EvalDataMany.hmlMadeColTotalFlop[i] += EvalDataMany.hmlMadeFlop[i][j];
						}
		}
		for (int i = 0; i < HML_FLOP_SIZE; i++) {
			for (int j = 0; j < MADE_SIZE; j++) {
