 ...
 * 
 * Draws that we check for:
 *	int DRAW_NONE = 1;
 *	int DRAW_BOARD_PAIR = 2;
 *	int DRAW_PAIRED_WITH_BOARD = 3;
 *	int DRAW_GUTSHOT = 4;
 *	int DRAW_STRAIGHT_DRAW = 5;
 *	int DRAW_OESD = 6;
 *	int DRAW_FLUSH_DRAW = 7;
 *	int DRAW_FLUSH_DRAW_OESD = 8;
 *	int DRAW_MAX_HANDS = 9;
 * 
 * @author PEAK_
 * 
 ***************************************************************************************/
...
public class DrawBoth implements Constants {
	...
	*/
	 ...
	 * Hole cards are already in cards so we just add 3 more.
	 */
	private boolean addCardsOfPair() {
		if (cards[0].value == EvalData.board[0].value || cards[1].value == EvalData.board[0].value) {
			cards[size++] = EvalData.board[0];
			this.taken[0] = true;
		}
		if (cards[0].value == EvalData.board[1].value || cards[1].value == EvalData.board[1].value) {
			cards[size++] = EvalData.board[1];
			this.taken[1] = true;
		}
		if (cards[0].value == EvalData.board[2].value || cards[1].value == EvalData.board[2].value) {
			cards[size++] = EvalData.board[2];
			this.taken[2] = true;
		}
		if (cards[0].value == EvalData.board[3].value || cards[1].value == EvalData.board[3].value) {
			cards[size++] = EvalData.board[3];
			this.taken[3] = true;
		}
		if (this.size >= 5) {
			return true;
		...
		if (this.taken[0] != true) {
			cards[size++] = EvalData.board[0];
		}
		if (this.size >= 5) {
			return true;
		}
		if (this.taken[1] != true) {
			cards[size++] = EvalData.board[1];
		}
		if (this.size >= 5) {
			return true;
		}
		if (this.taken[2] != true) {
			cards[size++] = EvalData.board[2];
		}
		if (this.size >= 5) {
			return true;
		}
		if (this.taken[3] != true) {
			cards[size++] = EvalData.board[3];
		}
	 ...
	 * Add 3 cards
	 * 
	 * Try to add cards that are sequential 
	 * If there are not enough cards return false; z
	 */
	private boolean addCardsOfStraight() {
		if (EvalData.board[0].value - EvalData.board[1].value == 1) {
			cards[this.size++] = EvalData.board[0];
			cards[this.size++] = EvalData.board[1];
			if (EvalData.board[1].value - EvalData.board[2].value == 1) {
				cards[this.size++] = EvalData.board[2];
			}
		} else if (EvalData.board[1].value - EvalData.board[2].value == 1) {
			cards[this.size++] = EvalData.board[1];
			cards[this.size++] = EvalData.board[2];
			if (EvalData.board[2].value - EvalData.board[3].value == 1) {
				cards[this.size++] = EvalData.board[3];
			}
		}
		if (this.size < 5) {
			return false;
	...
	/*-
	 * Determines if the cards have a flush draw.
	 * There must be 4 cards of same suit
	 * Returns true or false.
	 */
	private boolean areCardsFlushDraw() {
		int count = 0;
		for (int i = 0; i < 5; i++) {
	 ...
	 * Returns true or false.
	 */
	private boolean areCardsStraightDraw() {
		int count = 0;
		for (int i = 0; i < 5; i++) {
			if (this.cards[i].suit != this.cards[i + 1].suit) {
				++count;
