...
public class IndexArrayReport implements Constants {
	 ...
	 * Arg1 - Row for frame
	****************************************************************************************************************/
	void reportDraw(int c, int r,String title, int [][]indexArray, String[] nameArray ) {
		if (frame1 == null) {
		...
		for (int i = 0; i < indexArray.length; ++i) {
			table1.setValueAt(nameArray[i], row, 0);
			col = 1;
		 
			for (int j = 0; j < DRAW_COL; ++j) {
				if (IndexArrays.hmlDrawFlop[i][j] > 0) {
					double per = ((double) indexArray[i][j] / (double) IndexArrays.hmlCountFlop) * 100.0;
	 ...
	 * Same as above but for Made hands
	 ********************************************************************************** */
	void reportShowdown(int c, int r) {
		if (frame3 == null) {
		...
		for (int i = 0; i < HML_FLOP_SIZE; ++i) {
			...
			for (int j = 0; j < MADE_COL; ++j) {
				if (IndexArrays.hmlShowdownFlop[i][j] > 0) {
					double per = ((double) IndexArrays.hmlShowdownFlop[i][j] / (double) IndexArrays.showdownCount)
							* 100.0;
					sum += per;
					sumx += per;
