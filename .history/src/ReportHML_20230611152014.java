    ...
    static void reportDrawFlop(int c, int r) {
        ...
        for (int i = 0; i < HML_FLOP_SIZE; ++i) {
            ...
            for (int j = 0; j < DRAW_COL; ++j) {
                        System.out.println("//per "+IndexArrays.hmlDrawFlop[i][j] + " " +i + " " + j +  " "  + HML_FLOP_SIZE );
     ...
     //  double  per = ((double) IndexArrays.hmlDrawColTotalFlop[i] / (double) IndexArrays.hmlCountFlop) * 100.0;
     //    table1.setValueAt(Format.formatPer(per), row, i+1);
        }
        System.out.println("draw sumx " + sumx + " " + row);
