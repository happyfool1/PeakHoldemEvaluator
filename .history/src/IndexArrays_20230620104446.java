...
for (int i = 0; i < HML_FLOP_SIZE; i++) {
			for (int j = 0; j < DRAW_SIZE; j++) {
				for (int k = 0; k < MADE_SIZE; k++) {
					hmlDrawFlopToMadRiver[i][j][k] = 0;
					hmlDrawFlopToWon[i][j][k] = 0;;
				}
			}
		}
for (int i = 0; i < HML_FLOP_SIZE; i++) {
			for (int j = 0; j < MADE_SIZE; j++) {
				for (int k = 0; k < MADE_SIZE; k++) {
					hmlMadeFlopToMadRiver[i][j][k] = 0;
					hmlMadeFlopToWon[i][j][k] = 0;;
				}
			}
		}
for (int i = 0; i < HML_TURN_SIZE; i++) {
			for (int j = 0; j < DRAW_SIZE; j++) {
				for (int k = 0; k < MADE_SIZE; k++) {
					hmlDrawTurnToMadRiver[i][j][k] = 0;
					hmlDrawTurnToWon[i][j][k] = 0;;
				}
			}
		}
for (int i = 0; i < HML_TURN_SIZE; i++) {
			for (int j = 0; j < MADE_SIZE; j++) {
				for (int k = 0; k < MADE_SIZE; k++) {
					hmlMadeurnToMadRiver[i][j][k] = 0;
					hmlMadeTurnoToWon[i][j][k] = 0;;
				}
			}
		}
	...
	* +1 because of Showdown
	...
	static int[][][] hmlDrawFlopToMaWon = new int[HML_FLOP_SIZE][DRAW_SIZE][MADE_SIZE];
