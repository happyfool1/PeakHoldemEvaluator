...
/*- 
	...
	*
	...
	private static void playRiver() {
		...
		// ERROR Play() street not complete after 4 orbits EvalData.foldCount 4
		Logger.logError(new StringBuilder().append("ERROR Play()   street not complete after 4 orbits ")
				.append(" EvalData.foldCount ").append(EvalData.foldCount).append(" handsPlayed ").append(EvalData.handsPlayed)
				.toString());
		if (Options.krash) {
			Crash.log("$$$");
		}
	...
	private static void flopOrbit() {
		...
		if (!EvalData.playerFolded[seatBB] && !EvalData.playerAllin[seatBB]) {
			...
			if (EvalData.orbit == 0) {
				++EvalData.flopCountPos[EvalData.BB];
			}
			flopPlay();
		}
		...
		if (!EvalData.playerFolded[seatUTG] && !EvalData.playerAllin[seatUTG]) {
			...
			if (EvalData.orbit == 0) {
				++EvalData.flopCountPos[EvalData.UTG];
			}
			flopPlay();
		}
		...
		if (!EvalData.playerFolded[seatMP] && !EvalData.playerAllin[seatMP]) {
			...
			if (EvalData.orbit == 0) {
				++EvalData.flopCountPos[EvalData.MP];
			}
			flopPlay();
		}

		if (!EvalData.playerFolded[seatCutoff] && !EvalData.playerAllin[seatCutoff]) {
			EvalData.seat = seatCutoff;
			if (EvalData.orbit == 0) {
				++EvalData.flopCountPos[EvalData.CUTOFF];
			}
			flopPlay();
		}

		if (!(!EvalData.playerFolded[seatButton] && !EvalData.playerAllin[seatButton])) {
			return;
		}
		EvalData.seat = seatButton;
		if (EvalData.orbit == 0) {
	...
	*/
	private static void turnOrbit() {
		if (!EvalData.playerFolded[seatSB] && !EvalData.playerAllin[seatSB]) {
			EvalData.seat = seatSB;
			if (EvalData.orbit == 0) {
				++EvalData.turnCountPos[EvalData.SB];
			}
			turnPlay();
		}
		...
		if (!EvalData.playerFolded[seatBB] && !EvalData.playerAllin[seatBB]) {
			EvalData.seat = seatBB;
			if (EvalData.orbit == 0) {
				++EvalData.turnCountPos[EvalData.BB];
			}
			turnPlay();
		}
		...
		if (!EvalData.playerFolded[seatUTG] && !EvalData.playerAllin[seatUTG]) {
			EvalData.seat = seatUTG;
			if (EvalData.orbit == 0) {
				++EvalData.turnCountPos[EvalData.UTG];
			}
			turnPlay();
		}
		...
		if (!EvalData.playerFolded[seatMP] && !EvalData.playerAllin[seatMP]) {
			EvalData.seat = seatMP;
			if (EvalData.orbit == 0) {
				++EvalData.turnCountPos[EvalData.MP];
			}
			turnPlay();
		}
		...
		if (!EvalData.playerFolded[seatCutoff] && !EvalData.playerAllin[seatCutoff]) {
			EvalData.seat = seatCutoff;
			if (EvalData.orbit == 0) {
				++EvalData.turnCountPos[EvalData.CUTOFF];
			}
			turnPlay();
		}
		...
		EvalData.seat = seatButton;
		if (EvalData.orbit == 0) {
			++EvalData.turnCountPos[EvalData.BUTTON];
		}
		turnPlay();
	}
	...
	private static void riverOrbit() {
		if (!EvalData.playerFolded[seatSB] && !EvalData.playerAllin[seatSB]) {
			EvalData.seat = seatSB;
			if (EvalData.orbit == 0) {
				++EvalData.riverCountPos[EvalData.SB];
			}
			riverPlay();
		}
		...
		if (!EvalData.playerFolded[seatBB] && !EvalData.playerAllin[seatBB]) {
			EvalData.seat = seatBB;
			if (EvalData.orbit == 0) {
				++EvalData.riverCountPos[EvalData.BB];
			}
			riverPlay();
		}
		...
		if (!EvalData.playerFolded[seatUTG] && !EvalData.playerAllin[seatUTG]) {
			EvalData.seat = seatUTG;
			if (EvalData.orbit == 0) {
				++EvalData.riverCountPos[EvalData.UTG];
			}
			riverPlay();
		}
		...
		if (!EvalData.playerFolded[seatMP] && !EvalData.playerAllin[seatMP]) {
			EvalData.seat = seatMP;
			if (EvalData.orbit == 0) {
				++EvalData.riverCountPos[EvalData.MP];
			}
			riverPlay();
		}
		...
		if (!EvalData.playerFolded[seatCutoff] && !EvalData.playerAllin[seatCutoff]) {
			EvalData.seat = seatCutoff;
			if (EvalData.orbit == 0) {
				++EvalData.riverCountPos[EvalData.CUTOFF];
			}
			riverPlay();
		}
		...
		EvalData.seat = seatButton;
		if (EvalData.orbit == 0) {
			++EvalData.riverCountPos[EvalData.BUTTON];
		}
		riverPlay();
	...
	private static void flopPlay() {
		...
		++EvalData.flopCount[EvalData.seat];
		handIndex = EvalData.handIndex[EvalData.seat];

		if (EvalData.betType == EvalData.CHECK) {
			++EvalData.flopToMeCheck[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET1) {
			++EvalData.flopToMeBet1[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET2) {
			++EvalData.flopToMeBet2[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET3) {
			++EvalData.flopToMeBet3[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET4) {
			++EvalData.flopToMeBet4[EvalData.seat];
		} else if (EvalData.betType == EvalData.ALLIN) {
			++EvalData.flopToMeAllin[EvalData.seat];
		} else {
			Logger.logError("ERROR Play() invalid betType " + EvalData.betType);
			if (Options.krash) {
				Crash.log("$$$");
			}
		}
	...
	private static void turnPlay() {
		...
		if (EvalData.betType == EvalData.CHECK) {
			++EvalData.turnToMeCheck[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET1) {
			++EvalData.turnToMeBet1[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET2) {
			++EvalData.turnToMeBet2[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET3) {
			++EvalData.turnToMeBet3[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET4) {
			++EvalData.turnToMeBet4[EvalData.seat];
		} else if (EvalData.betType == EvalData.ALLIN) {
			++EvalData.turnToMeAllin[EvalData.seat];
		} else {
			Logger.logError("ERROR Play() invalid betType " + EvalData.betType);
			if (Options.krash) {
				Crash.log("$$$");
	...
	private static void riverPlay() {
		...
		if (EvalData.betType == EvalData.CHECK) {
			++EvalData.riverToMeCheck[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET1) {
			++EvalData.riverToMeBet1[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET2) {
			++EvalData.riverToMeBet2[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET3) {
			++EvalData.riverToMeBet3[EvalData.seat];
		} else if (EvalData.betType == EvalData.BET4) {
			++EvalData.riverToMeBet4[EvalData.seat];
		} else if (EvalData.betType == EvalData.ALLIN) {
			++EvalData.riverToMeAllin[EvalData.seat];
		} else {
			Logger.logError("ERROR Play() invalid betType " + EvalData.betType);
			if (Options.krash) {
				Crash.log("$$$");
...
/*-
	...
	private static void playHandFlop() {
		...
		// Next we try C Bet
		if (Options.CBet[EvalData.seat] && EvalData.seat == EvalData.preflopRaiser && EvalData.betType == EvalData.CHECK) {
			EvalData.decisionProcess[EvalData.streetNumber][EvalData.seat][EvalData.orbit] = EvalData.DEC_CBET;
			if (playHandCBet()) {
				return;
			}
		}

		final boolean condition = (EvalData.betType == EvalData.CHECK || EvalData.betType == EvalData.BET1
				|| EvalData.betType == EvalData.BET2) && (Options.mdfFlop[EvalData.seat] || Options.mdfBet2Flop[EvalData.seat])
				&& (EvalData.betType == EvalData.CHECK || EvalData.betType == EvalData.BET1 || EvalData.betType == EvalData.BET2)
				&& (Options.mdfFlop[EvalData.seat] || Options.mdfBet2Flop[EvalData.seat]) && playHandMDFFlop();
	...
	private static void playHandTurn() {
		...
		if (Options.barrelTurn[EvalData.seat] && barrelTurn()) {
			EvalData.decisionProcess[EvalData.streetNumber][EvalData.seat][EvalData.orbit] = EvalData.DEC_BARREL;
			return;
		}

		final boolean condition = (EvalData.betType == EvalData.CHECK || EvalData.betType == EvalData.BET1
				|| EvalData.betType == EvalData.BET2) && (Options.mdfTurn[EvalData.seat] || Options.mdfBet2Turn[EvalData.seat])
				&& playHandMDFTurn();
	...
	private static void playHandRiver() {
		...
		// Next we try barreling
		if (Options.barrelRiver[EvalData.seat] && barrelRiver()) {
			EvalData.decisionProcess[EvalData.streetNumber][EvalData.seat][EvalData.orbit] = EvalData.DEC_BARREL;
			return;
		}

		final boolean condition = (EvalData.betType == EvalData.CHECK || EvalData.betType == EvalData.BET1
				|| EvalData.betType == EvalData.BET2) && (Options.mdfRiver[EvalData.seat] || Options.mdfBet2River[EvalData.seat])
				&& playHandMDFRiver();
...
/*-
	...
	private static boolean playHandMDFFlopCheck() {
		...
		if (EvalData.seat == seatCutoff && EvalData.strategy[stratIndex].flopMDFCutoff.rangeArray[handIndex] > 0) {
			doRaise(87, POSTFLOP_MDF);
			++EvalData.mdfRaiseCountFlop[EvalData.seat];
			return true;
		}
	...
	private static boolean playHandMDFFlopBet1() {
		...
		if (EvalData.seat == seatCutoff && EvalData.strategy[stratIndex].flopMDFBet2Cutoff.rangeArray[handIndex] > 0) {
			doRaise(55, POSTFLOP_MDFBET2);
			++EvalData.mdfRaiseBet2CountFlop[EvalData.seat];
			return true;
		}
		...
		if (EvalData.betType == EvalData.BET1 && Options.mdfFlop[EvalData.seat]) {
			...
			if (EvalData.seat == seatCutoff && EvalData.strategy[stratIndex].flopMDFCallCutoff.rangeArray[handIndex] > 0) {
				doCallCheckFold();
				++EvalData.mdfCallCountFlop[EvalData.seat];
				return true;
			}
	...
	*/
	private static boolean playHandCBet() {
		...
		if (Evaluate.evalArray[Evaluate.BOARD_WET]) {
			...
			if (EvalData.seat == seatCutoff && EvalData.strategy[stratIndex].flopCBetWetCutoff.rangeArray[handIndex] > 0) {
				doCBetWet();
				return true;
			}
		...
		if (Evaluate.evalArray[Evaluate.BOARD_DRY]) {
			...
			if (EvalData.seat == seatCutoff && EvalData.strategy[stratIndex].flopCBetDryCutoff.rangeArray[handIndex] > 0) {
				doCBetDry();
				return true;
			}
		...
		if (Evaluate.evalArray[Evaluate.BOARD_NOT_WET_DRY]) {
			...
			if (EvalData.seat != seatCutoff
					&& EvalData.strategy[stratIndex].flopCBetNeutralCutoff.rangeArray[handIndex] > 0) {
				doCBetNeutral();
				return true;
			}
