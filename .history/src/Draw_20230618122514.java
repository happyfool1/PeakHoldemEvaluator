	...
	private void finish(int type) {
		...
		if (this.size >= 4) {
			...
			if (EvalData.street == FLOP) {
				System.out.println("ZZZ Flop "+this.type);
				EvalData.drawTypeFlop[EvalData.seat] = this.type;
			} else if (EvalData.street == TURN) {
				System.out.println("ZZZ Turn "+this.type);
				EvalData.drawTypeTurn[EvalData.seat] = this.type;
			}
		}else{if (EvalData.street == FLOP) {
				System.out.println("ZZZ Flop error "+this.type);
				EvalData.drawTypeFlop[EvalData.seat] = DRAW_NONE;
			} else if (EvalData.street == TURN) {
				System.out.println("ZZZ Turn error "+this.type);
				EvalData.drawTypeTurn[EvalData.seat] = DRAW_NONE;
			}
			return;
		}
			System.out.println("ZZZ All "+this.type);
		holeCardsFirst();
	}
