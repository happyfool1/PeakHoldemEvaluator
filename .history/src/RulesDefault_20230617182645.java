...
public class RulesDefault implements Constants {
	/*-
	...
	private static void autoRiverNIT() {
		for (int i = 0; i < Rules.HANDS; ++i) {
			for (int k = 0; k < Rules.OP_ACTIONS; ++k) {
				...
				if (i == Evaluate.POCKET_PAIR_BELOW_TP) {
					if (k == Rules.OP_CHECK) {
						ruleArray[i][k] = Rules.NO_ACTION;
					}
					if (k == Rules.OP_BET1) {
	...
	private static void autoFlopFish() {
		for (int i = 0; i < Rules.HANDS; ++i) {
			for (int k = 0; k < Rules.OP_ACTIONS; ++k) {
				...
				if (i == Evaluate.FLUSH_DRAW_GUTSHOT) {
					if (k == Rules.OP_CHECK) {
						ruleArray[i][k] = Rules.BET;
					}
					if (k == Rules.OP_BET1) {
