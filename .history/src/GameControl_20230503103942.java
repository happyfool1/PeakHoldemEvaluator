...
public class GameControl implements Constants {
	...
	**************************************************************************** */
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < PLAYERS; j++) {
				for (int k = 0; k < 4; k++) {
					...
					potStart$[i][j][k]   = zeroBD;
	
	 potEnd$[i][j][k]  =  = zeroBD;
	bet$[i][j][k]  = zeroBD;
	 moneyIn$[i][j][k]    = zeroBD;
	 stack$[i][j][k]  = zeroBD;
	side1$  [i][j][k]         = zeroBD;
	 side2$ [i][j][k]  = zeroBD;
	 side3$[i][j][k]   = zeroBD;
	 ...
	 call$ [i][j][k]  = zeroBD;
	 allin$ [i][j][k]  = zeroBD;
	 returnedTo$ [i][j][k]  = zeroBD;
	 raisedTo$ [i][j][k] = zeroBD;
	 calledTo$ [i][j][k]  = zeroBD;
	 win$ [i][j][k]  = zeroBD;
	 returnedToShowdown$[i][j][k] zeroBD;
	...
	private static void playerBetBB() {
		...
		EvalData.stackBD[0][seat][EvalData.orbit] = EvalData.stacBD[seat];
		EvalData.moneyInBD[0][seat][EvalData.orbit] = EvalData.moneyInBD[seat];
	}
