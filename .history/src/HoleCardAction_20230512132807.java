...
public class HoleCardAction implements Constants {
    ...
    *  Construct panel. 
    ...
    void constructPanel() {
       ...
       panelLimp.setBorder(open);
       for (int i = 0; i < POSITIONS; ++i) {
           openText[i][0] = new JTextField(String.valueOf(actionArrayPlayabilityCall[i][0]));
           openText[i][0].setFont(f0);
           openText[i][0].setSize(40, 40);
           panelLimp.add(openText[i][0]);
       }
       for (int i = 0; i < POSITIONS; ++i) {
             openText[i][1] = new JTextField(Format.formatPer(actionArrayPercentageCall[i][0] ));
           openText[i][1].setFont(f0);
           openText[i][1].setSize(40, 40);
           panelLimp.add(openText[i][1]);
       }
       for (int i = 0; i < POSITIONS; ++i) {
           openText[i][2] = new JTextField(String.valueOf(actionArrayCombosCall[i][0]));
           openText[i][2].setFont(f0);
           openText[i][2].setSize(40, 40);
           panelLimp.add(openText[i][2]);
       }
       for (int i = 0; i < POSITIONS; ++i) {
           openText[i][3] = new JTextField(String.valueOf(actionArrayGapsCall[i][0]));
           openText[i][3].setFont(f0);
           openText[i][3].setSize(40, 40);
           panelLimp.add(openText[i][3]);
       }
