...
public class IndexArray implements Constants {
    ...
    ***************************************************************************************** */
    void findBestAndWorst() {
      ...
      Integer minNumber = drawTotalRowPerEntries.stream().min(Comparatorhttps://www.appsdeveloperblog.com/streams-max-and-min-operations/#:~:text=stream()%0A%20%20%20%20%20%20%20%20%20%20%20%20.min(Comparator-,.comparing(Integer%3A%3AvalueOf)),-.get()%3B%0A%0A%20%20%20%20System.out.getRowOrCon();\

        Optional<PercentageEntry> minEntry = drawTotalRowPerEntries.stream()
                .min(Comparator.comparing(PercentageEntry::getPercentage));

        if (minEntry.isPresent()) {
            System.out.println("Minimum percentage: " + minEntry.get().getPercentage());
        } else {
            System.out.println("The list is empty.");
        }

        // Get the largest percentage in the list
        Optional<PercentageEntry> maxEntry = drawTotalRowPerEntries.stream()
                .max(Comparator.comparing(PercentageEntry::getPercentage));

        if (maxEntry.isPresent()) {
            System.out.println("Maximum percentage: " + maxEntry.get().getPercentage());
        } else {
            System.out.println("The list is empty.");
        }
