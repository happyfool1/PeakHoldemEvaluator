  ...
  ******************************************************************************/
  static BigDecimal adjustPreflopBetSize(BigDecimal  bet) {
    ...
    if (EvalData.betType == PREFLOP_BET1){
      ...
      adjust = adjust.multiply(EvalDatas.BBBetSizeBD)
      return bet.multiply(EvalData.BBBetSize,add(adjustmentBet1BetLimps[EvalData.limpCount].add(adjustmentBet1BetsFolds[EvalData.foldCount]));  
    }  
    if (EvalData.betType == PREFLOP_BET2){
      return bet.add(adjustmentBet2BetLimps[EvalData.limpCount].add(adjustmentBet2BetsFolds[EvalData.foldCount]));  
    } 
    if (EvalData.betType == PREFLOP_BET3){
      return bet.add(adjustmentBet3BetLimps[EvalData.limpCount].add(adjustmentBet3BetsFolds[EvalData.foldCount]));   
    }
    return zeroBD;
  }
