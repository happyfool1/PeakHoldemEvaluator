...
public class IndexArrayReport implements Constants {
	...
	****************************************************************************************************************/
	 ...
	 ********************************************************************************** */
	void reportMade		if (frame1 == null) {

		if (frame2 == null) {
			frame2 = new JFrame(" HML Made hands");
		...
		for (int i = 0; i < HML_FLOP_SIZE; ++i) {
			...
			for (int j = 0; j < MADE_COL; ++j) {
				if (IndexArrays.hmlMade[i][j] > 0) {
					double per = ((double) IndexArrays.hmlMade[i][j] / (double) IndexArrays.hmlCount) * 100.0;
					sum += per;
					sumx += per;
	 ...
	 * Same as above but for Made hands
	 ********************************************************************************** */
	void reportShowdown		if (frame1 == null) {

		if (frame3 == null) {
			frame3 = new JFrame("Showdown  HML Winner hands");
