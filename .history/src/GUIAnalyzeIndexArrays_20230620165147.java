...
public class GUIAnalyzeIndexArrays extends JFrame implements ActionListener, Constants {
    ...
    * a radio button or control button.
    ...
    private void doSimulationDataCollection() {
        ...
        IndexArray hml = new IndexArray(IndexArrays.hmlDrawFlop, IndexArrays.hmlMadeFlop, IndexArrays.hmlShowdownFlop,
                ...
                MADE_ARRAY_ST);
        IndexArrayReport report = new IndexArrayReport();

        if (hmlDrawFlop.isSelected()) {
            report.reportDraw(10, 200, "Flop Draws with HML Index", hml);
        }
        if (hmlMadeFlop.isSelected()) {
            report.reportMade(10, 220, "Flop  Made hands with HML Index", hml);

        }
        if (hmlShowdownFlop.isSelected()) {
            report.reportShowdown(10, 240, "Flop  Showdown hands with HML Index", hml);
    ...
    *  ****************************************************************************** */
    static boolean checkExit(int played) {
        if (statusUpdate++ > 10000) {
            statusUpdate = 0;
            status.setText(Integer.valueOf(played).toString());
            status.repaint();
             panelRadioHands.repaint();
            System.out.println(Integer.valueOf(played).toString());
        }
        if (quit) {
            return true;
        }
