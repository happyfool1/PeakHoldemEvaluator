...
public class GUIAnalyzeMany extends JFrame implements ActionListener, Constants {
    ...
    ***************************************************************************** */
    ...
    private static final JCheckBox typeShowdown = new JCheckBox("Type Showdown");
    ...
    * a radio button or control button.
    ...
    private void doSimulationDataCollection() {
        ...
        Evaluate.doSimulationDataCollection(number);
        if (hmlMade.isSelected()) {
            // ReportHML.reportHMLFlopMade(100, 100);
        }
        if (hmlDraw.isSelected()) {
            // ReportHML.reportHMLFlopDraw(750, 100);
        }
        if (hmlShowdown.isSelected()) {
            // ReportHML.reportHMLShowdownFlopWon(150, 150);
        }
        if (hhmlDraw.isSelected()) {
            // ReportHML.reportHMLTurnDraw(750, 100);
        }
        if (hhmlMade.isSelected()) {
            // ReportHML.reportHMLTurnMade(750, 100);
        }
        if (hhmlShowdown.isSelected()) {
            // ReportHML.reportHHMLShowdownTurnWon(150, 150);
        }
        if (hhhmlDraw.isSelected()) {
            // ReportHML.reportHMLRiverDraw(750, 100);
        }
        if (hhhmlMade.isSelected()) {
            // ReportHML.reportHMLRiverMade(750, 100);
        }
        if (hhhmlShowdown.isSelected()) {
            // ReportHML.reportHHHMLShowdownRiverWon(150, 150);
        }
        ReportHML.reportHMLMade(100, 100);
        ReportHML.reportHMLDraw(750, 100);
        ReportHML.reportHMLShowdownFlopWon(150, 150);

        ReportHML.reportHMLMade(100, 100);
        ReportHML.reportHMLDraw(750, 100);
        ReportHML.reportHMLShowdownTurnWon(150, 150);



        ReportHML.reportHMLShowdownFlopWon(150, 150);
        ReportHML.reportHMLShowdownTurnWon(200, 150);
        ReportHML.reportHMLShowdownRiverWon(250, 150);
    }
}
