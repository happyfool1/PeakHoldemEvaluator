...
public class Showdown implements Constants {
	...
	******************************************************************************/
	static void showdownValue(int seat) {
		...
		System.out.println("Max value " + MADE_ARRAY_ST[max] + " maxValue " + EvalData.maxValue);
	...
	static void showdown() {
		...
		System.out.println("showdown() Max value "+ max + " " + MADE_ARRAY_ST[max] + " ");
		for (int i = 0; i < PLAYERS; i++) {
			if (!EvalData.playerFolded[i] && EvalData.showdownRank[i] != -1) {
				if (EvalData.showdownRank[i] < max) {
					...
					System.out.println("showdown() Player " + i + " won Showdown " + EvalData.showdownHand[i]);
				}
			}
		}
