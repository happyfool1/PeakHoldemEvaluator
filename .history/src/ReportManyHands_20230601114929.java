	...
	********************************************************************************** */
	...
	static void reportHHMLDraw(int c, int r) {
		if (frame3 == null) {
			...
			tableModel3 = new DefaultTableModel(dataHHMLDraw, columnsHMLDraw);
		...
		for (int i = 0; i < HML_FLOP_SIZE; ++i) {
			...
			for (int j = 0; j < DRAW_HANDS_SIZE; ++j) {
				if (ManyData.hmlDraw[i][j] > 0) {
					table3.setValueAt("   " + String.valueOf(ManyData.hmlDraw[i][j]), row, col);
					double per = ((double) ManyData.hmlDraw[i][j] / (double) ManyData.hmlCount) * 100.0;
	...
	********************************************************************************** */
	static void reportHHMLMade(int c, int r) {
		if (frame4 == null) {
			...
			tableModel4 = new DefaultTableModel(dataHMLMade, columnsHMLMade);
		...
		for (int i = 0; i < HML_FLOP_SIZE; ++i) {
			...
			for (int j = 0; j < MADE_HANDS_SIZE; ++j) {
				if (ManyData.hmlMade[i][j] > 0) {
					table4.setValueAt("   " + String.valueOf(ManyData.hmlMade[i][j]), row, col);
					double per = ((double) ManyData.hmlMade[i][j] / (double) ManyData.hmlCount) * 100.0;
	...
	/*-**********************************************************************************
		 ...
		 * Arg1 - Row for frame
		********************************************************************************** */
	static void reporHHHMLDraw(int c, int r) {
		if (frame5 == null) {
			frame5 = new JFrame("River HML DRaw hands");
			frame5.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
			frame5.setLocation(c, r);
			frame5.setPreferredSize(new Dimension(650, 600));
			tableModel5 = new DefaultTableModel(dataHMLDraw, columnsHMLDraw);
		...
		for (int i = 0; i < HML_FLOP_SIZE; ++i) {
			...
			for (int j = 0; j < DRAW_HANDS_SIZE; ++j) {
				if (ManyData.hmlDraw[i][j] > 0) {
					table5.setValueAt("   " + String.valueOf(ManyData.hmlDraw[i][j]), row, col);
					double per = ((double) ManyData.hmlDraw[i][j] / (double) ManyData.hmlCount) * 100.0;
					table5.setValueAt(Format.formatPer(per), row + 1, col);
				}
				col++;
			}
			col = 1;
	...
	********************************************************************************** */
	static void reportHHHMLMade(int c, int r) {
		if (frame6 == null) {
			...
			tableModel6 = new DefaultTableModel(dataHMLMade, columnsHMLMade);
		...
		for (int i = 0; i < HML_FLOP_SIZE; ++i) {
			...
			for (int j = 0; j < MADE_HANDS_SIZE; ++j) {
				if (ManyData.hmlMade[i][j] > 0) {
					table6.setValueAt("   " + String.valueOf(ManyData.hmlMade[i][j]), row, col);
					double per = ((double) ManyData.hmlMade[i][j] / (double) ManyData.hmlCount) * 100.0;
