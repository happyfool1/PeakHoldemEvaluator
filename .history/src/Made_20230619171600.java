...
public class Made implements Constants {
	...
	*
	...
	private void finish(int type) {
		...
		if (size > 4) {
		//		System.out.println("KKK " + size + " " + cards[0] + cards[1] + cards[2] + cards[3] + cards[4] + " "
	...
	private int boardPairs() {
		...
		if (EvalData.boardSet && isSet()) {
			if (EvalData.threeOfKind && isSet()) {
			System.out.println("Set2 " + this.cards[0] + this.cards[1] + this.cards[2] + this.cards[3] + this.cards[4]);
			return MADE_SET;
		}
	...
	* Is this a Royal Flush
	...
	private void addBiggerCards() {
		...
		//	System.out.println("LLL1 " + size + " " + cards[0] + cards[1] + cards[2] + cards[3] + cards[4] + " "
		//			+ EvalData.handsPlayed);
		for (int i = 0; i < EvalData.bothCount; i++) {
			if (!taken[i]) {
				if (this.kicker1 == -1) {
				...
				taken[i] = true;
				if (this.size >= 5) {
			//			System.out.println("LLL2 " + size + " " + cards[0] + cards[1] + cards[2] + cards[3] + cards[4] + " "
			//		+ EvalData.handsPlayed);
					return;
				}
			}
