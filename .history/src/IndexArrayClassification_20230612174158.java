...
public class IndexArrayClassification implements Constants {
	...
	***************************************************************************************************/
	...
	static void initialize() {
		EvalData.hmlCount = EvalData.flopCount = 0;
		for (int i = 0; i < HML_FLOP_SIZE; ++i) {
			EvalData.riverCount[i] =EvalData.turnCount[i] = 0;
		}
	}
	...
	static void showdownData(int i0, boolean b1) {
		++EvalData.showdownCount[i0];
		if (b1) {
			++EvalData.showdownWin[i0];
		} else {
			++EvalData.showdownLoose[i0];
		}
