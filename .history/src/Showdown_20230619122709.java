	...
	******************************************************************************/
	static void showdownValue(int seat) {
		...
		if (max == MADE_ROYAL_FLUSH) {
			...
			System.out.println("CCC " +  EvalData.kicker1 + " " + EvalData.kicker2 + " "  + EvalData.kicker3 + " " + EvalData.handsPlayed);
	...
	static void showdown() {
		...
		for (int i = 0; i < PLAYERS; i++) {
			if (!EvalData.playerFolded[i]) {
				System.out.println("BBB " + max + " " + EvalData.showdownRank[i] + " " + i);
				if (EvalData.showdownRank[i] > max) {
					max = EvalData.showdownRank[i];
					if (max == -1) {
						System.out.println("AAA " + max + " " + EvalData.showdownRank[i] + " " + i);
					}
				}
			} else {
