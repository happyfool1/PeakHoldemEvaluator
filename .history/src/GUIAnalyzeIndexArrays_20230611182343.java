...
public class GUIAnalyzeIndexArrays extends JFrame implements ActionListener, Constants {
    ...
    * a radio button or control button.
    ...
    private void doSimulationDataCollection() {
        ...
        ( IndexArrays.hmlDrawFlop,IndexArrays.hmlMadeFlop,IndexArrays.hmlShowdownFlop,HML_FLOP_ST);
    IndexArrayReport report = new IndexArrayReport();

        if (hmlDrawFlop.isSelected()) {
            report.reportDraw(10,100,"Flop Draws with HML Index",hml);
            ReportHML.reportDrawFlop(10, 100);
        }
        if (hmlMadeFlop.isSelected()) {
             report.reportMade(10,100,"Flop  Made hands with HML Index",hml);
            ReportHML.reportMadeFlop(10, 120);
        }
        if (hmlShowdownFlop.isSelected()) {

            Re  report.reportMade(10,100,"Flop  Made hands with HML Index",hml);portHML.reportShowdownFlop(10, 140);
        }
