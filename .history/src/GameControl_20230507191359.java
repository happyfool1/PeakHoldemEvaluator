...
private static void uncalledBet() {
	final int active = PLAYERS - Table.foldCount;
	if (!Table.betCalled && Table.betType == CHECK) {
		return;
	}

	if (Table.handsPlayed == 1893350) {
		Logger.log(new StringBuilder().append("//noSide  ").append(" \tactive ").append(active).append(" \tshort ")
				.append(Table.callAllinShortCount).append(" \tbetCalled ").append(Table.betCalled).toString());
	}
	if (Table.side1) {
		return;
	}
	if (active == 2 && Table.callAllinShortCount == 1) {
		doTheReturnCall();
		return;
	}
	if (active == 1 && Table.betNow != 0 || (!Table.betCalled && Table.allinCount == active)) {
		doTheReturnBet();
	}
