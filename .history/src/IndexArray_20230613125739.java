...
public class IndexArray implements Constants {
    ...
    ****************************************************************************** */
    IndexArray(int[][] drawArray, int[][] madeArray, int[][] showdownArray, String[] rowNames, String[] colNames) {
        ...
        this.drawArrayRowNames = new String[ rowNames.length];
        this.madeArrayRowNames = new String[ rowNames.length];
        this.showdownArrayRowNames = new String[rowNames.length];
        this.drawArrayColNames = new String[ colNames.length];
        this.madeArrayColNames = new String[ colNames.length];
        this.showdownArrayColNames = new String[colNames.length];
        this.drawArrayRowNames =rowNames.
    ...
    * name is a
    * value is a row or column value.
    ***************************************************************************************** */
    class ArrayListX {
        Double percentage;
        int value;

        public ArrayListX(Double percentage, int value) {
    ...
    List<PercentageEntry> entries = new ArrayList<>();
    ...
    ***************************************************************************************** */
    void createIndexArrayList() {
        for (int row = 0; row < drawArrayPer.length; row++) {
            for (int column = 0; column < drawArrayPer[row].length; column++) {
                entries.add(new PercentageEntry(drawArrayPer[row][column], row, column));
            }
        }
        entries.sort(Comparator.comparing(PercentageEntry::getPercentage));
        // For reversed
        entries.sort(Comparator.comparing(PercentageEntry::getPercentage).reversed());

        double targetPercentage = 0.5; // Replace with the percentage you're searching for
        List<PercentageEntry> matchingEntries = entries.stream()
                .filter(entry -> entry.getPercentage() == targetPercentage)
                .collect(Collectors.toList());

        List<PercentageEntry> matchingEntriesX = entries.stream()
    ...
    public double getPercentageByRowAndColumn(int targetRow, int targetColumn) {
        for (PercentageEntry entry : entries) {
    ...
    ***************************************************************************************** */
    ...
    String [] drawArrayRowNames;
    String [] madeArrayRowNames;
    String [] showdownArrayRowNames;
    String [] drawArrayColNames;
    String [] madeArrayColNames;
    String [] showdownArrayColNames;
    
    // List for each array
    // List<ArrayList> drawArrayListRow = new ArrayList<>();
    // List<ArrayList> madeArrayListRow = new ArrayList<>();
    // List<ArrayList> showdownArrayListRow = new ArrayList<>();
