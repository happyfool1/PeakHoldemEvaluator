...
public class UpdateEvalDataMany implements Constants {
	...
	static void updateSummary() {
		...
		for (int j = 0; j < DRAW_SIZE; j++) {
			for (int i = 0; i < HML_FLOP_SIZE; i++) {
				 EvalDataMany.hmlDrawColTotalFlop[j] += EvalDataMany.hmlDrawFlop[i][j];
			}
		}
		for (int i = 0; i < HML_FLOP_SIZE; i++) {
		...
		for (int j = 0; j < MADE_SIZE; j++) {
			for (int i = 0; i < HML_TURN_SIZE; i++) {
						EvalDataMany.hmlMadeColTotalTurn[j] += EvalDataMany.hmlMadeTurn[i][j];
			}
		}
		for (int i = 0; i < HML_TURN_SIZE; i++) {
			for (int j = 0; j < MADE_SIZE; j++) {
		...
		for (int j = 0; j < MADE_SIZE; j++) {
			for (int i = 0; i < HML_TURN_SIZE; i++) {
				EvalDataMany.hmlShowdownColTotalTurn[j] += EvalDataMany.hmlShowdownTurn[i][j];
			}
		}
		...
		for (int j = 0; j < MADE_SIZE; j++) {
			for (int i = 0; i < HML_RIVER_SIZE; i++) {
				EvalDataMany.hmlShowdownColTotalRiver[java.awt.dnd] += EvalDataMany.hmlShowdownRiver[j][i];
			}
		}
