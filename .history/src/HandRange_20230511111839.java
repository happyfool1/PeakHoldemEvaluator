...
public class HandRange implements Constants {
	...
	double [] percent = new double[PLAYERS];
	...
	private static boolean rangeRead;
	...
	****************************************************************************** */
	...
	/*-******************************************************************************
	 * Create  playabilityArrayOrder  TODO 
	...
	/*-******************************************************************************
	 * Values are handIndex  TODO ?? into playabilityArray, an array of String
	 * orderArray is with value handIndex
	****************************************************************************** */
	 ...
	 *	int[] gapsArray = new int[HANDS]; // Gaps in HandRange value is handIndex 
	 *******************************************************************************/
	void updateForHoleCardAction() {
		// Find lowest rangeArray element with a hand
			for (int i = HANDS - 1; i >= 0; i--) {
			if (this.rangeArray[i] >= 0) {
				this.lowRangeArrayIndex = i;
				this.lowPlayabilityArrayNumericValue = playabilityArrayNumeric[i];
				this.cards = handsArray[i];
			//	System.out.println(
			//			"//HR1 " + i + " " + handsArray[i] + " " + rangeArray[i] + " " + lowRangeArrayIndex + " "
			//					+ lowPlayabilityArrayNumericValue + " " + cards + " " 
			//					+ playabilityArray[lowPlayabilityArrayNumericValue ]);
				break;
			}
		}
		...
		for (int i = HANDS - 1; i >= 0; i--) {
			if (orderArray[i] == lowRangeArrayIndex) {
				...
				//System.out.println("//HR2 " + i + " " + orderArray[i]);
				break;
			}
		}
				this.gapCount = 0;
		if (lowRangeArrayIndex > 1) {
			for (int i = lowRangeArrayIndex - 1; i >= 0; i--) {
				if (this.rangeArray[i] >= 0) {
					this.gapsArray[this.gapCount++] = i;
				//	System.out.println("//HR3 " + i + " " + orderArray[i] + " " + this.gapsArray[this.gapCount + -1]);
				}
			}
		}
		//System.out.println("//HR3 " + gapCount);
		}
	...
	* Create 3 arrays
	 ...
	 * 		orderArray
	 * Called by constructor when an instance is created
	 * playabilityArrayOrder and orderArray are indexed by handIndex
	 * playabilityArrayNumeric is indexrd by 169 =- k which is ?? TODO
	 * 
	 * playabilityArrayNumeric value is ?? indexed by ?? TODO
	 * playabilityArrayOrder value is ?? indexed by ?? TODO
	 * orderArray value is ?? indexed by ?? TODO
	 * 
	 * The arrays will contain values from 1 - 169
	 * AA is 169, 72o is 1
	  ***************************************************************************/
