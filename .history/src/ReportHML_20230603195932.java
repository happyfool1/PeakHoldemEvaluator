   ...
   * Arg1 - Row for frame
    ****************************************************************************************************************/
    ...
    static void reportHMLMadeFlop(int c, int r) {
        ...
        tableModel2 = new DefaultTableModel(dataHMLDrawFlop, columnsHMLDrawFlop);
        ...
        for (int i = 0; i < HML_FLOP_SIZE; ++i) {
            ...
            for (int j = 0; j < DRAW_HANDS_SIZE; ++j) {
                if (ManyData.hmlDraw[i][j] > 0) {
                     double per = ((double) ManyData.hmlDraw[i][j] / (double) ManyData.hmlCount) * 200.0;
