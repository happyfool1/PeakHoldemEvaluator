...
public class UpdateEvalDataMany implements Constants {
	...
	static void updateSummary() {
		...
		for (int j = 0; j < DRAW_SIZE; j++) {
			for (int i = 0; i < HML_FLOP_SIZE; i++) {
				System.out.println("//zzz " + i + " " + j + " " + EvalDataMany.hmlDrawRowTotalFlop
				EvalDataMany.hmlDrawColTotalFlop[i] += EvalDataMany.hmlDrawFlop[j][i];
			}
		}
		for (int i = 0; i < HML_FLOP_SIZE; i++) {
			for (int j = 0; j < MADE_SIZE; j++) {
				EvalDataMany.hmlMadeRowTotalFlop[i] += EvalDataMany.hmlMadeFlop[i][j];
				EvalDataMany.hmlMadeSumOfAllValuesFlop += EvalDataMany.hmlMadeFlop[i][j];
			}
		}

		for (int j = 0; j < MADE_SIZE; j++) {
			for (int i = 0; i < HML_TURN_SIZE; i++) {
				EvalDataMany.hmlMadeColTotalTurn[i] += EvalDataMany.hmlMadeTurn[j][i];
			}
		}
		for (int i = 0; i < HML_TURN_SIZE; i++) {
			for (int j = 0; j < MADE_SIZE; j++) {
		...
		for (int j = 0; j < MADE_SIZE; j++) {
			for (int i = 0; i < HML_TURN_SIZE; i++) {
				EvalDataMany.hmlShowdownColTotalTurn[i] += EvalDataMany.hmlShowdownTurn[j][i];
			}
		}

		for (int j = 0; j < MADE_SIZE; j++) {
			for (int i = 0; i < HML_RIVER_SIZE; i++) {
				EvalDataMany.hmlMadeColTotalRiver[i] += EvalDataMany.hmlMadeRiver[i][j];
			}
		}

		for (int i = 0; i < HML_RIVER_SIZE; i++) {
			for (int j = 0; j < MADE_SIZE; j++) {
				EvalDataMany.hmlShowdownRowTotalRiver[i] += EvalDataMany.hmlShowdownRiver[i][j];
				EvalDataMany.hmlShowdownSumOfAllValuesRiver += EvalDataMany.hmlShowdownRiver[i][j];
			}
		}
		for (int j = 0; j < MADE_SIZE; j++) {
			for (int i = 0; i < HML_RIVER_SIZE; i++) {
				EvalDataMany.hmlShowdownColTotalRiver[i] += EvalDataMany.hmlShowdownRiver[j][i];
			}
		}
