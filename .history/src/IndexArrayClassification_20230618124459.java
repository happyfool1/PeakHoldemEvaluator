...
public class IndexArrayClassification implements Constants {
	...
	* Call only one time for each Flop
	...
	static void doFlop() {
		...
		hmlBoardFlop();
		if (EvalData.hmlIndexFlop == -1 || EvalData.hmlIndexFlop > 9) {
	...
	* Do Turn characterization
	...
	static void doTurn() {
		hmlBoardTurn();
			if (EvalData.hmlIndexTurn== -1 || EvalData.hmlIndexTurn > 9) {
			Logger.logError("ERROR HML Turn index invalid "+ EvalData.hmlIndexFlop);
