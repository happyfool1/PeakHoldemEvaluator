	...
	**************************************************************************** */
	...
	int getDecision() {
		...
		if (EvalData.betType == PREFLOP_LIMP) {
			if (this.range.Raise(position, EvalData.betType) <= this.handIndex) {
				doOpen();
				return PREFLOP_OPEN;

			}
			if (this.range.getEVCall(position, EvalData.betType) <= this.handIndex) {
				doLimp();
				return PREFLOP_LIMP;
			} else {
				doFold();
		...
		if (EvalData.betType == PREFLOP_OPEN) {
			if (this.range.getEVRaise(position, EvalData.betType) <= this.handIndex) {
				return PREFLOP_BET3;
			}
			if (this.range.getEVCall(position, EvalData.betType) <= this.handIndex) {
				doCall();
		...
		if (EvalData.betType == PREFLOP_BET4) {
			if (this.range.getEVRaise(position, EvalData.betType, 0) <= this.handIndex) {
				doAllin();
				return PREFLOP_ALLIN;
			}
			if (this.range.getEVCall(position, EvalData.betType, 1) <= this.handIndex) {
				doCallBet4();
				return PREFLOP_BET4;
			} else {
				doFold();
		...
		if (EvalData.betType == PREFLOP_ALLIN) {
			if (this.range.getEV(position, EvalData.betType) <= this.handIndex) {
				doAllinCall();
				return PREFLOP_ALLIN;
			} else {
				doFold();
