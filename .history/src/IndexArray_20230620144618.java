...
public class IndexArray implements Constants {
    ...
    ****************************************************************************** */
    IndexArray(int[][] drawArray, int[][] madeArray, int[][] showdownArray,
        ...
        copyArray(this.madeArray, madeArray);
        copyArray(this.drawArray, drawArray);
        copyArray(this.showdownArray, showdownArray);

        copyArray(this.allArrayRowNames, rowNames);
        copyArray(this.drawArrayColNames, drawColNames);
        copyArray(this.madeArrayColNames, madeColNames);
    ...
    ***************************************************************************************** */
    private void copyArray( int [][] fromArrat, int [] toArrat) {    ){ 
        for (int i = 0; i < fromArrat.length; i++) {
                toArraty[i][j] = fromArrat[i][j];
            }
        }
    }
     private void copyArray( String[][] fromArrat, String [][] toArrat) {    ){ 
        for (int i = 0; i < fromArrat.length; i++) {
            for (int j = 0; j < fromArrat[i].length; j++) {
                toArray[i][j] = fromArrat[i][j];
            }
        }
    }
  private void copyArray( String[] fromArrat, String [][] toArrat) {    ){ 
