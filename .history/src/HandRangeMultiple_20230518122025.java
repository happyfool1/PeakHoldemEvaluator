...
* the future may hold. 
* There at are 60 HandRange arrays in one instance of this Class.
* 
...
public class HandRangeMultiple implements Constants, java.io.Serializable {
    ...
    * gapcount is the number of gaps in a ramnge. A gap is when there is a hand  
    ...
    *******************************************************************************/
     static final int[] handsArrayPioSolver = { UTG, UTG, UTG, UTG, UTG, UTG, UTG, UTG, UTG, UTG, UTG, UTG, UTG,
            ...
            //produced by Bart 
    public static final double[] VALUES_DOUBLE = {
            1.522, 1.442, 1.291, 1.178, 1.154, 1.087, 1.062, 1.005, 0.980, 0.923, 0.908, 0.851, 0.826, 0.801, 0.744,
            0.729, 0.672, 0.647, 0.622, 0.597, 0.572, 0.547, 0.522, 0.497, 0.472, 0.447, 0.422, 0.397, 0.372, 0.347,
            0.322, 0.297, 0.272
    };

    public static final String[] _STRING = {
            "AA", "KK", "QQ", "AKs", "AQs", "AJs", "ATs", "KQs", "KJs", "QJs", "QTs", "JTs", "T9s", "T8s", "98s", "97s",
            "87s", "76s", "65s", "54s", "AKo", "ATo", "KQo", "KJo", "QJo", "JTo", "T9o", "87o", "76o", "65o", "54o"
    };
    ...
    *******************************************************************************/
     ...
     * few. 
     * 
     *Returns boolean decision to raise or not raise. false meand call or fold.
     *******************************************************************************/
    boolean decisionRaise(int handIndex, int pos, int toMe, int adjust) {
        System.out.println("//decisionRaise " + handIndex + " " + pos + " " + toMe + " " + adjust
                + " " + this.rangeArrayRaise[pos][toMe]);
        int h = this.rangeArrayRaise[pos][toMe].high;
        int x = playabilityArrayNumeric[h];
        if (this.rangeArrayRaise[pos][toMe].gapsCount == 0 || adjust != 0) {
            if (x <= (h + adjust)) {
                return true;
            } else {
                return false;
    ...
    * Make Raise decision based on playability alone if there are no gaps in  range.
    ...
    boolean decisionCall(int handIndex, int pos, int toMe, int adjust) {
        ...
        int x = playabilityArrayNumeric[h];
        if (this.rangeArrayCall[pos][toMe].gapsCount == 0 || adjust != 0) {
            if (x <= (h + adjust)) {
                return true;
            } else {
                return false;
    ...
    * Copy RangeOrder values to array   TODO 
    ...
    int getOrderIndex(int ndx) {
        return HandRangeMultiple.orderArray[ndx];
    }
    ...
    *  Construct panel. 
    ...
    void constructPanel(String title) {
        JTextField[][] openText = new JTextField[POSITIONS][4];
        JTextField[][] limpText = new JTextField[POSITIONS][4];

        JTextField[][] bet3Text = new JTextField[POSITIONS][4];
        JTextField[][] callText = new JTextField[POSITIONS][4];

        JTextField[][] bet4Text = new JTextField[POSITIONS][4];
        JTextField[][] bet3CallText = new JTextField[POSITIONS][4];

        JTextField[][] allinText = new JTextField[POSITIONS][4];
        JTextField[][] bet4CallText = new JTextField[POSITIONS][4];

        JTextField[][] allinCallText = new JTextField[POSITIONS][4];
    ...
    HandRangeMultiple readFromFile(String path) {
        System.out.println("readFromFile() path = " + path);
        ...
        if (r == null || r.rangeArrayRaise[2][0] == null) {
            System.out.println("Error readFromFile() handRangeMultiple = null");
        }
        return r;
    }
