    ...
    ****************************************************************************** */
    IndexArrayAnalysis(int[][] drawArray, int[][] madeArray, int[][] showdownArray) {
        ...
        for (int j = 0; j < this.drawArrayPer.length; j++) {
            for (int i = 0; i < this.drawArrayPer[0].length; i++) {
                this.drawColTotalPer[i] += this.drawArrayPer[i][j];
            }
        }
        for (int j = 0; j < this.madeArrayPer.length; j++) {
            for (int i = 0; i < this.madeArrayPer[0].length; i++) {
                this.madeColTotalPer[j] += this.madeArrayPer[j][i];
            }
        }
        for (int j = 0; j < this.showdownArrayPer.length; j++) {
            for (int i = 0; i < this.showdownArrayPer[0].length; i++) {
                this.showdownColTotalPer[i] += this.showdownArrayPer[i][j];
            }
        }

        for (int i = 0; i < this.drawArrayPer.length; i++) {
            for (int j = 0; j < this.drawArrayPer[0].length; j++) {
                this.drawRowTotalPer[j] += this.drawArrayPer[i][j];
