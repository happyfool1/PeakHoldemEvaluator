...
public class ReportHML implements Constants {
    ...
    ********************************************************************************** */
    static void reportHMLShowdownRiverWon(int c, int r) {
        ...
        reportHMLShowdownFlopWonX(300, 300);
    }

    /*-********************************************************************************
     * The method createAndShowGUI does the following:
     * 
     * 1. It first creates a new JFrame.
     * 2. Then it initializes a PriorityQueue to hold the cells with the five highest values.
     * 3. It iterates over the data array, checking each cell. If the PriorityQueue is not full, 
     ...
     * 4. It then creates a JTable, passing in the data and column arrays, and overriding the 
     *    prepareRenderer method to highlight cells with one of the five highest values.
     * 5. Finally, it adds the JTable to a JScrollPane, adds the JScrollPane to the JFrame, 
     *    and displays the JFrame.
     ***********************************************************************************/
    static void reportHMLShowdownFlopWonX(int c, int r) {
        if (frame7 == null) {
            frame7 = new JFrame("Showdown Flop HML Winner hands");
            frame7.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
            frame7.setLocation(c, r);
            frame7.setPreferredSize(new Dimension(1050, 600));
        }
        ...
        // Iterate over all cells in the data array
        for (int row = 0; row < dataHMLFlopShowdownWon.length; row++) {
            for (int column = 0; column < dataHMLFlopShowdownWon[0].length; column++) {
                if (largestCells.size() < 5) {
                    largestCells.offer(new Cell(row, column, (int) dataHMLFlopShowdownWon[row][column]));
                } else if ((int) dataHMLFlopShowdownWon[row][column] > largestCells.peek().value) {
                    largestCells.poll();
                    largestCells.offer(new Cell(row, column, (int) dataHMLFlopShowdownWon[row][column]));
                }
            }
        }
System.out.println(largestCells.size());
        // Create a JTable with a custom TableCellRenderer
        JTable table = new JTable(dataHMLFlopShowdownWon, columnsHMLFlopShowdownWon) {
            @Override
            public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
                Component c = super.prepareRenderer(renderer, row, column);

                if (containsCell(largestCells, row, column)) {
