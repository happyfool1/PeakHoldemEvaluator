        ...
        for (int i = 0; i < HML_FLOP_SIZE; ++i) {
            ...
            for (int j = 0; j < DRAW_COL; ++j) {
                if (EvalDataMany.hmlDrawFlop[i][j] > 0) {
                    double per = ((double) EvalDataMany.hmlDrawFlop[i][j] / (double) EvalDataMany.hmlCount) * 100.0;
                    sum += per;
                    sumx += per;
    ...
    ********************************************************************************** */
    static void reportShowdownRiver(int c, int r) {
        ...
        for (int i = 0; i < HML_RIVER_SIZE; ++i) {
            ...
            sum = 0.0;
            for (int j = 0; j < MADE_COL; ++j) {
                if (EvalDataMany.hhhmlShowdownRiver[i][j] > 0) {
                    double per = ((double) EvalDataMany.hhhmlShowdownWon[i][j] / (double) EvalDataMany.showdownCount) * 100.0;
                    sum += per;
                    sumx += per;
                    table8.setValueAt(Format.formatPer(per), row, col);
                    table8.setValueAt(Format.formatPer(sum), row, MADE_TOTAL_COL);
                    String cellValueStr = "   " + String.valueOf(EvalDataMany.hhhmlShowdownWon[i][j]);
                    // Parse the cell value to an integer and add to the priority queue
                    try {
                        int cellValue = Integer.parseInt(cellValueStr.trim());
                        // For smallest cells
