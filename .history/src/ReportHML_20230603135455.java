    ...
    static void reportHMLShowdownFlopWon(int c, int r) {
        if (frame10 == null) {
            ...
            for (int row = 0; row < dataHMLFlopShowdownWon.length; row++) {
                // Start from column 1 to exclude column 0
                for (int column = 1; column < dataHMLFlopShowdownWon[0].length; column++) {
                    // Get value from the cell, parse it to integer and assign to the variable
                    int cellValue = Integer.parseInt((String)dataHMLFlopShowdownWon[row][column]);
    
                    if (largestCells.size() < 5) {
                        largestCells.offer(new Cell(row, column, cellValue));
