...
public class IndexArrayClassification implements Constants {
	...
	* or hand.   For example:
	* The Flop is evaluated as a combination of High, Middle, and Low EbalData.board.
...
*******************************************************************************/
private static int flopTexturePairBroadway( ) {
        
        // Determine pair distribution type
        int pairType;
        if (EbalData.board[0].value == EbalData.board[1].value && EbalData.board[1].value == EbalData.board[2].value) {
            pairType = 1; // Trips
        } else if (EbalData.board[0].value == EbalData.board[1].value || EbalData.board[1].value == EbalData.board[2].value || EbalData.board[0].value == EbalData.board[2].value) {
            pairType = 2; // Pair
        } else {
            pairType = 3; // Unpaired
        }
