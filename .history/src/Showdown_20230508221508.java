...
public class Showdown implements Constants {
	...
	***************************************************************************************/
	...
	static void showdown() {
		...
		if (EvalData.manyHands) {
			// TODO EvaluateMany.updateShowdown();
		}
	}
	 ...
	 * 
	 * Noth that the numbers used like 804000 are arbitrary and were chosen only to
	...
	/*-
	 * Initialize for showdownValue 
	 */
	private static void initializeShowdownValue() {
		EvalData.showdown = false;
		EvalData.winner = false;
		EvalData.winnerSeat = -1;
		EvalData.maxValue = -1;
		EvalData.kicker3 = EvalData.kicker2 = EvalData.kicker1 = 0;
	...
	static void playerData() {
		...
		case MADE_STRAIGHT_FLUSH -> {
			EvalData.highCards1[EvalData.seat] = EvalData.highCard1;
		...
		case MADE_FULL_HOUSE -> {
			EvalData.highCards1[EvalData.seat] = EvalData.highCard1;
			EvalData.highCards2[EvalData.seat] = EvalData.highCard2;
			// TODO EvalData.showdownSt[EvalData.seat] = new StringBuilder().append("//Fill
			// house high card ")
			// .append(CARDS_REVERSE[EvalData.highCard1]).append(" pair high card ")
			// .append(CARDS_REVERSE[EvalData.highCard2]).toString();
		}
		case MADE_FLUSH -> {
			EvalData.highCards1[EvalData.seat] = EvalData.highCard1;
			EvalData.suits[EvalData.seat] = EvalData.suit;
			EvalData.showdownSt[EvalData.seat] = new StringBuilder().append("//Flush high card  ")
					.append(Card.CARDS_REVERSE[EvalData.highCard1]).append(" suit ").append(SUITS2_ST[EvalData.suit])
					.toString();
		}
		case MADE_STRAIGHT -> {
			EvalData.highCards1[EvalData.seat] = EvalData.highCard1;
			// TODO EvalData.showdownSt[EvalData.seat] = "//Straight high card " +
			// CARDS_REVERSE[EvalData.highCard1];
		}
		case MADE_SET -> {
			copyKickers();
			EvalData.highCards1[EvalData.seat] = EvalData.highCard1;
	...
	*  Example  MIDDLE_PAIR to HAND_MIDDLE_PAIR
	 *  Hands like MIDDLE_PAIR are mostly about draws and rlative strength.
	...
	/*- *****************************************************************************
	 * A method to determine the best made hand in a sorted array 
	 * of 7 Card objects:  
	 * Returns the best hand, -1 if error.
	 * TODO  code is a mess
	 ***************************************************************************** */
	private static int getBestHand() {
		if (EvalData.madeArray[MADE_STRAIGHT_FLUSH]) {
			return MADE_STRAIGHT_FLUSH;
		}
		if (EvalData.madeArray[MADE_FOUR_OF_A_KIND]) {
			return MADE_FOUR_OF_A_KIND;
		}
		if (EvalData.madeArray[MADE_FULL_HOUSE]) {
			return MADE_FULL_HOUSE;
		}
		if (EvalData.madeArray[MADE_FLUSH]) {
			return MADE_FLUSH;
		}
		if (EvalData.madeArray[MADE_STRAIGHT]) {
			return MADE_STRAIGHT;
		}
		if (EvalData.madeArray[MADE_SET]) {
			return MADE_SET;
		}
		if (EvalData.madeArray[MADE_TWO_PAIR]) {
		...
		if (EvalData.madeArray[MADE_NONE]) {
			return MADE_NONE;
		}
		return -1;
	}
