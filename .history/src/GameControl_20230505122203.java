...
public class GameControl implements Constants {
	...
	**************************************************************************** */
	...
	private static void playHandPreflop() {
		...
		return;
	}System.out.println("//playHandPreflop()A "+betToMe);action=players[seat].getDecision(betType,betToMe,betNow);betType=action.actionAfter;betNow=action.betNow;betToMe=action.betNow;System.out.println("//playHandPreflop()B "+betToMe);if(action.action==FOLD)
	...
	*******************************************************************************/
	private static void preflopOrbit1() {
		seat = seatSB;
		playHandPreflop();
		System.out.println("//PPPP  " + foldCount + " " + EvalData.foldCount);
		
		if (EvalData.foldCount == MAXFOLDED || streetComplete()) {
			System.out.println("//preflopOrbit1A()  " + foldCount);
			return;
		}
		seat = seatBB;
		playHandPreflop();
		if (EvalData.foldCount == MAXFOLDED || streetComplete()) {
			System.out.println("//preflopOrbit1B()  " + foldCount);
			return;
		}
		...
		if (!playerFolded[seatUTG] && !playerAllin[seatUTG]) {
			...
			if (foldCount == MAXFOLDED || streetComplete()) {
				System.out.println("//preflopOrbit1C()  " + foldCount);
				return;
			}
		}
		...
		if (!playerFolded[seatMP] && !playerAllin[seatMP]) {
			...
			if (EvalData.foldCount == MAXFOLDED || streetComplete()) {
				System.out.println("//preflopOrbit1D()  " + foldCount);
				return;
			}
		}
		seat = seatCO;
		...
		if (!EvalData.playerFolded[seatCO] && !playerAllin[seatCO]) {
			if (EvalData.foldCount == MAXFOLDED || streetComplete()) {
				System.out.println("//preflopOrbit1E()  " + foldCount);
				return;
			}
		}
		seat = seatBU;
		...
		if (!(!EvalData.playerFolded[seatBU] && !playerAllin[seatBU])) {
			System.out.println("//preflopOrbit1F()  " + foldCount);
			return;
		}
		
		System.out.println("//preflopOrbit1G()  " + foldCount);
	}
	...
	*******************************************************************************/
	private static void preflopOrbit23() {
		...
		// Play hand Big Blind
		if (!pEvalData.layerFolded[seatBB] && !playerAllin[seatBB]) {
			seat = seatBB;
			playHandPreflop();
		}
	...
	private static boolean streetComplete() {
		...
		System.out.println("streetCompleteA " + EvalData.foldCount);
