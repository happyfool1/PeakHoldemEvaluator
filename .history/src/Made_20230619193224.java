...
* 
* A  Classes Draw is very similar to this Class.
...
* to avoid loops.
* A second version of the combined array is sorted first by suit then by value 
* and used for Flushes.
* 
* A made hand is possible only if it includes both hole cards.
* We verify that here.
* 
...
printpublic class Made implements Constants {
	int type;
	...
	*
	...
	int doHandStraightFlush() {
		result = doFlush();
		if (result == MADE_NONE) {
			this.type = MADE_NONE;
		}
		if (this.cards[0].value - this.cards[1].value == 1 && this.cards[1].value - this.cards[2].value == 1 &&
				this.cards[2].value - this.cards[3].value == 1 && this.cards[3].value - this.cards[4].value == 1) {
			result = MADE_STRAIGHT_FLUSH;
			if (this.cards[0].value == ACE) {
				result = MADE_ROYAL_FLUSH;
			}
		}
		finish(result);
		return result;
	...
	int doHandFullOrQuad() {
		if (EvalData.fourOfKind && is4Of()) {
			final int result = MADE_NONE;
			this.type = MADE_FOUR_OF_A_KIND;
			finish(result);
			return result;
		}

		if (EvalData.fullHouse && isFullHouse()) {
			final int result = MADE_FULL_HOUSE;
			finish(result);
			return result;
		}
		this.type = MADE_NONE;
	...
	private void finish(int type) {
		...
		if (EvalData.street == FLOP) {
			EvalData.madeTypeFlop[EvalData.seat] = type;
		}
		if (EvalData.street == TURN) {
			EvalData.madeTypeTurn[EvalData.seat] = type;
		} else {
			EvalData.madeTypeRiver[EvalData.seat] = type;
		}
		if (this.size < 5) {
			addBiggerCards();
		}
	 ...
	 * Check board first, for board paired, two pair, or set
	 ***************************************************************************************/
	private int doPairs() {
		...
		if (EvalData.threeOfKind && isSet()) {
			System.out.println("Set1 " + this.cards[0] + this.cards[1] + this.cards[2] + this.cards[3] + this.cards[4]
					+ " " + EvalData.threeOfKind);
		}
		return MADE_NONE;
	}

	private int boardPairs() {
		...
		if (EvalData.boardSet && isSet()) {
			...
			System.out.println(
					"Set2 " + this.cards[0] + this.cards[1] + this.cards[2] + this.cards[3] + this.cards[4] + " "
							+ EvalData.boardSet);
	...
	* If the pair is a board pair it does not count? TODO
	/*- ************************************************************************************/
	int whatKindOfPair() {
	...
	* Is this a Royal Flush
	...
	private void addBiggerCards() {
		...
		// System.out.println("LLL1 " + size + " " + cards[0] + cards[1] + cards[2] +
		// cards[3] + cards[4] + " "
		// + EvalData.handsPlayed);
		for (int i = 0; i < EvalData.bothCount; i++) {
			if (!taken[i]) {
				...
				taken[i] = true;
				if (this.size >= 5) {
					// System.out.println("LLL2 " + size + " " + cards[0] + cards[1] + cards[2] +
					// cards[3] + cards[4] + " "
					// + EvalData.handsPlayed);
					return;
				}
	...
	* Does cards have a set Returns true or false
	...
	private boolean isCardsFourOfAKind() {
		if (this.cards[0].value == this.cards[1].value && this.cards[0].value == this.cards[2].value
				&& this.cards[0].value == this.cards[3].value) {
			return true;
		}
		if (this.cards[1].value == this.cards[2].value && this.cards[1].value == this.cards[3].value
	...
	private boolean isCardsStraight() {
		...
		if (this.cards[1].value == FIVE && this.cards[2].value == FOUR
	...
	boolean isCardsFlush() {
		if (this.cards[0].suit == this.cards[1].suit && this.cards[0].suit == this.cards[2].suit
