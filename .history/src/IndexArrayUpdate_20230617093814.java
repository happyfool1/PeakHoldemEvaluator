...
public class IndexArrayUpdate implements Constants {
	...
	static void updateFlop() {
		...
		for (int i = 0; i < PLAYERS; i++) {
			if (!EvalData.playerFolded[i]) {
				if (EvalData.drawTypeFlop[i] != -1) {
					++IndexArrays.hmlDrawFlop[index][EvalData.drawTypeFlop[i]];
					++IndexArrays.drawFlop[EvalData.seat];
				}
				if (EvalData.madeTypeFlop[i] != -1) {
					++IndexArrays.hmlMadeFlop[index][EvalData.madeTypeFlop[i]];
	...
	static void updateTurn() {
		...
		if (EvalData.drawTypeFlop[EvalData.seat] != -1) {
	...
	static void updateRiver() {
		...
		for (int i = 0; i < PLAYERS; i++) {
			if (!EvalData.playerFolded[i]) {
				++IndexArrays.hmlMadeRiver[index][EvalData.madeTypeFlop[i]];
				++IndexArrays.madeRiver[i];
			}
		}
