 ...
 * @author PEAK_
  ...
  ********************************************************************************** */
  ...
  for (int i = 0; i < HML_FLOP_SIZE; ++i) {
      ...
      for (int j = 0; j < MADE_HANDS_SIZE; ++j) {
          if (ManyData.hmlMade[i][j] > 0) {
              table11.setValueAt("   " + String.valueOf(ManyData.hhhmlShowdownWon[i][j]), row, col);
              double per = ((double) ManyData.hhhmlShowdownWon[i][j] / (double) ManyData.hhhmlCount) * 100.0;
  ...
  ********************************************************************************** */
  ...
  if (frame12 == null) {
      ...
      frame12.setPreferredSize(new Dimension(650, 600));
  ...
  for (int i = 0; i < HML_FLOP_SIZE; ++i) {
      ...
      for (int j = 0; j < MADE_HANDS_SIZE; ++j) {
          if (ManyData.hmlMade[i][j] > 0) {
              table12.setValueAt("   " + String.valueOf(ManyData.hhhmlShowdownWon[i][j]), row, col);
              double per = ((double) ManyData.hhhmlShowdownWon[i][j] / (double) ManyData.hhhmlCount) * 100.0;
              table12.setValueAt(Format.formatPer(per), row + 1, col);ManyData.showdownCount
          }
          col++;
      }
      col = 1;
