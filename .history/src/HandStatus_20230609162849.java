...
public class HandStatus implements Constants {
	...
	*******************************************************************************/
	static void findPairsSetsFullHouse() {
		...
		if (numPairs == 2) {
			System.out.println("HandStatus 2 pair " +numPairs);
		}
		// If the pair found is a board pair it does not count As a pair
		if (numPairs == 1 && EvalData.boardPair) {
			return;
		}
		if (numPairs == 2 && EvalData.boardTwoPair) {
			return;
		}
		if (numSet == 1 && EvalData.boardSet) {
			return;
		}
		...
		for (int i = 0; i < EvalData.bothCount - 1; i++) {
			for (int j = i + 1; j < EvalData.bothCount; j++) {
				if (EvalData.both[i].equals(EvalData.both[j])) {
					Logger.logError("checkForNoDuplicates() Duplicate cards in EvalData.both " + EvalData.both[i]);
				}
			}
		}
	}
