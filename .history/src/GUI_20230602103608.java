...
public class GUI extends JFrame implements ActionListener, Constants {
	...
	*******************************************************************************/
	public void myFrame() {
		...
		panelShowdown.setBounds(20, 500, 1100, 50);
		mainPanel.add(panelShowdown);

		panelShowdownValue = new JPanel(new GridLayout(1, 1));
		panelShowdownValue.setBounds(20, 560, 1100, 50);
		...
		showdown.setFont(fs);
		showdown.setPreferredSize(new Dimension(500, 25));
		showdown.setBackground(SHOW);
		panelShowdown.add(showdown);

		showdownValue.setFont(fs);
		showdownValue.setPreferredSize(new Dimension(500, 25));
		showdownValue.setBackground(VALUE);
	...
	private void doExecute() {
		...
		for (int i = 0; i < PLAYERS; i++) {
			if (seatActive[i]) {
				if (selectedStreet == FLOP) {
					...
					Evaluate.doRiver(i);
					showdownValue.setText(Showdown.showdownValueString());
				}
				if (i == selectedSeat) {
					// doReport();
		...
		if (selectedStreet == SHOWDOWN || selectedStreet == PREFLOP) {
			Evaluate.doShowdown();
