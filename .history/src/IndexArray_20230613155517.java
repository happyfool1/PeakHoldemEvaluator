...
public class IndexArray implements Constants {
    ...
    ***************************************************************************************** */
    ...
    List<PercentageEntry> drawArrayEntries = new ArrayList<>();
    List<PercentageEntry> madeArrayEntries = new ArrayList<>();
    ...
    void createIndexArrayList() {
        for (int row = 0; row < drawArrayPer.length; row++) {
            for (int column = 0; column < drawArrayPer[row].length; column++) {
                drawArrayEntries.add(new PercentageEntry(drawArrayPer[row][column], row, column));
            }
        }
        drawEntries.sort(Comparator.comparing(PercentageEntry::getPercentage));
        // For reversed
        // drawEntries.sort(Comparator.comparing(PercentageEntry::getPercentage).reversed());
        for (int row = 0; row < madeArrayPer.length; row++) {
            for (int column = 0; column < madeArrayPer[row].length; column++) {
                madeArrayEntries.add(new PercentageEntry(madeArrayPer[row][column], row, column));
            }
        }
        drawEntries.sort(Comparator.comparing(PercentageEntry::getPercentage));
