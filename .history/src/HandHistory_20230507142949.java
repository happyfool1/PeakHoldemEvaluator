...
class HandHistory implements Constants {
	...
	* SECOND Board [9c 4h 7d 6c 7c]
	...
	{if(sidePotBD.compareTo(zeroBD)!=0){reportSt[lines++]=new StringBuilder().append("Total pot $").append(Format.format(EvalData.potBD)).append(st).toString();}else{mainPotBD=EvalData.potBD.subtract(sidePotBD);reportSt[lines++]=new StringBuilder().append("Total pot $").append(Format.format(EvalData.potBD)).append(" Main pot $").append(Format.format(mainPotBD)).append(". Side pot $").append(Format.format(sidePotBD)).append(st).toString();}}if(EvalData.streetNumber==FLOP){reportSt[lines++]=new StringBuilder().append("Board [").append(EvalData.board[0]).append(" ").append(EvalData.board[1]).append(" ").append(EvalData.board[2]).append("]\r\n").toString();}if(EvalData.streetNumber==TURN){reportSt[lines++]=new StringBuilder().append("Board [").append(EvalData.board[0]).append(" ").append(EvalData.board[1]).append(" ").append(EvalData.board[2]).append(" ").append(EvalData.board[3]).append("]\r\n").toString();}if(EvalData.streetNumber==RIVER){reportSt[lines++]=new StringBuilder().append("Board [").append(EvalData.board[0]).append(" ").append(EvalData.board[1]).append(" ").append(EvalData.board[2]).append(" ").append(EvalData.board[3]).append(" ").append(EvalData.board[4]).append("]\r\n").toString();}
