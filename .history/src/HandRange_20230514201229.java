...
public class HandRange implements Constants, java.io.Serializable {
	...
	Gaps gaps = new Gaps(); // Gaps in HandRange value is handIndex
	...
	******************************************************************************/
	private void findGaps(int high) {
		if (debug) {
			System.out.println("findGaps() path "+ high + " " + path);
		}
		this.gapsCount = 0;
		for (int i = 0; i < HANDS; ++i) {
			if (rangeArray[i] < 0) {
				if (playabilityArrayNumeric[i] < high) {
					gaps.gapsArray[this.gapsCount++] = i;
					if (this.debug) {
					//findGaps() 38 high 140 -1 numeric 84 Q2s
//findGaps() 48 high 140 -1 numeric 81 J5s
//findGaps() 49 high 140 -1 numeric 90 J4s
//findGaps() 50 high 140 -1 numeric 94 J3s
//findGaps() 51 high 140 -1 numeric 99 J2s	
						System.out.println(
								"//findGaps() " + i + " high " + high + " " + rangeArray[i]
										+ " numeric " + playabilityArrayNumeric[i] + " " + handsArray[i]);
					}
				}
			}
		}
		gaps.gapCount = this.gapsCount;
	}
	...
	private void getRangeHigh() {
		...
		this.high = -1;
		for (int i = HANDS - 1; i >= 0; i--) {
			if (this.rangeArray[i] > 0 && orderArrayNumeric[i[this.rangeArray[i]] > ndx) {
