...
public class Showdown implements Constants {
	...
	******************************************************************************/
	static void showdownValue(int seat) {
		int max = m
		EvalData.highCard1 = EvalData.hand[seat].highCard1;ax =EvalData.madeTypeRiver[seat];
		...
		} else if (max == MADE_BOARD_PAIR_PLUS_UNDER_PAIR ||max == MADE_BOARD_PAIR_PLUS_OVER_PAIR || max == MADE_BOTTOM_TWO_PAIR ||
		 max == MADE_TOP_TWO_PAIR || max == MADE_TOP_AND_BOTTOM_TWO_PAIR || max == MADE_BOTTOM_PAIR || max == MADE_BOARD_TWO_PAIR) {
	...
	static void showdown() {
		...
		if (EvalData.manyHands) {
			IndexArrayClassification.doShowdown();
		}
	}
