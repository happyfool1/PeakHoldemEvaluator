...
public class IndexArrayUpdate implements Constants {
	...
	static void updateFlop() {
		...
		System.out.println("XXX " + index + " " + EvalData.seat + " " +EvalData.drawTypeFlop[EvalData.seat] );
	...
	//	System.out.println("xxx " + index + " " + EvalData.seat + " " +EvalData.drawTypeTurn[EvalData.seat] );
		...
		for (int i = 0; i < PLAYERS; i++) {
			if (!EvalData.playerFolded[i] && EvalData.playerWonShowdown[i]) {
				...
				if (EvalData.hmlIndexRiver != -1) {
					...
					System.out.println("XXX " +EvalData.hmlIndexFlop+ " " + EvalData.showdownHand[i] + " " + i);
