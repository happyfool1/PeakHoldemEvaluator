...
public class GameControl implements Constants {
	...
	*******************************************************************************/
	...
	static int first = 0; // TODO
	static int rulePlay = 0; // TODO
	static int last = 0; // TODO
	static int RULEHU = 0; // TODO
	static int RULE = 0; // TODO
	static int POSTFLOP_RULE = 0; // TODO
	static int POSTFLOP_BLUFF = 0; // TODO
...
/*- **************************************************************************** 
	...
	*  Return uncalled bet
	 ...
	 * TODO
	 * Play Flop.   
	 * In a future version it will make the evaluation of Turn hands more accuratge
	 * because some weak hands will be folded. If done correctly it will make the evaluation of 
	 * Turn hands more accuratge because some weak hands will be folded as in a real game. 
	 * The first implementation may be to simply fold some percentage of hands that do not hit
	 * the Flop. Phase 2 will be to add the Rules code borrowed from PeakHoldemSimulation.
	 * Not perfect, but better.
	 * Lots of code commented out, will maybe be re-written in the future.
	 *
	 ******************************************************************************************** */
	...
	private static void getRulePLay() {
		...
		if (EvalData.seat == first) {
			if (EvalData.foldCount - EvalData.PLAYERS == 2) {
				...
				if (EvalData.street== EvalData.FLOP) {
					// rulePlay =
					// EvalData.strategy[stratIndex].flopFirstRules.ruleArray[handValue[EvalData.seat]][EvalData.betType];
					return;
				}
				if (EvalData.street== EvalData.TURN) {
					// rulePlay =
					// EvalData.strategy[stratIndex].turnFirstRules.ruleArray[handValue[EvalData.seat]][EvalData.betType];
					return;
				}
				// rulePlay =
				// EvalData.strategy[stratIndex].riverFirstRules.ruleArray[handValue[EvalData.seat]][EvalData.betType];
				return;
			}
			// Not heads up
			...
			// rulePlay =
			// EvalData.strategy[stratIndex].riverFirstRulesHU.ruleArray[handValue[EvalData.seat]][EvalData.betType];
			return;
		}
		...
		if (EvalData.seat == last) {
			if (EvalData.foldCount - EvalData.PLAYERS == 2) {// Heads up
				...
				if (EvalData.street== EvalData.FLOP) {
					// rulePlay =
					// EvalData.strategy[stratIndex].flopLastRules.ruleArray[handValue[EvalData.seat]][EvalData.betType];
					return;
				}
				if (EvalData.street== EvalData.TURN) {
					// rulePlay =
					// EvalData.strategy[stratIndex].turnLastRules.ruleArray[handValue[EvalData.seat]][EvalData.betType];
					return;
				}
				// rulePlay =
				// EvalData.strategy[stratIndex].riverLastRules.ruleArray[handValue[EvalData.seat]][EvalData.betType];
				return;
			}
			// Not heads up
			// EvalData.rule[EvalData.seat] = RULE;
			...
			// rulePlay =
			// EvalData.strategy[stratIndex].riverLastRulesHU.ruleArray[handValue[EvalData.seat]][EvalData.betType];
			return;
		}
		...
		if (EvalData.street== EvalData.FLOP) {
			// rulePlay =
			// EvalData.strategy[stratIndex].flopMiddleRules.ruleArray[handValue[EvalData.seat]][EvalData.betType];
			return;
		}
		if (EvalData.street== EvalData.TURN) {
			// rulePlay =
			// EvalData.strategy[stratIndex].turnMiddleRules.ruleArray[handValue[EvalData.seat]][EvalData.betType];
			return;
		}
		// rulePlay =
		// EvalData.strategy[stratIndex].riverMiddleRules.ruleArray[handValue[EvalData.seat]][EvalData.betType];
	}
	...
	******************************************************************************************* */
	private static boolean playHandMDFFlopCheck() {
		...
		// EvalData.decisionProcessRule[EvalData.streetNumber][EvalData.seat][EvalData.orbit]
		// = EvalData.DEC_MDFCHECK;
		// if (EvalData.seat == seatButton &&
		// EvalData.strategy[stratIndex].flopMDFButton.rangeArray[handIndex] > 0) {
		// doRaise(87, POSTFLOP_MDF);
		// ++EvalData.mdfRaiseCountFlop[EvalData.seat];
		// return true;
		// }
		// if (EvalData.seat == seatCO &&
		// EvalData.strategy[stratIndex].flopMDFCO.rangeArray[handIndex] > 0) {
		// doRaise(87, POSTFLOP_MDF);
		// ++EvalData.mdfRaiseCountFlop[EvalData.seat];
		// return true;
		// }
		// if (EvalData.seat == seatMP &&
		// EvalData.strategy[stratIndex].flopMDFMP.rangeArray[handIndex] > 0) {
		// doRaise(87, POSTFLOP_MDF);
		// ++EvalData.mdfRaiseCountFlop[EvalData.seat];
		// return true;
		// }
		// if (EvalData.seat == seatUTG &&
		// EvalData.strategy[stratIndex].flopMDFUTG.rangeArray[handIndex] > 0) {
		// doRaise(87, POSTFLOP_MDF);
		// ++EvalData.mdfRaiseCountFlop[EvalData.seat];
		// return true;
		// }
		// if (EvalData.seat == seatBB &&
		// EvalData.strategy[stratIndex].flopMDFBB.rangeArray[handIndex] > 0) {
		// doRaise(87, POSTFLOP_MDF);
		// ++EvalData.mdfRaiseCountFlop[EvalData.seat];
		// return true;
		// }
		// if (!(EvalData.seat == seatSB &&
		// EvalData.strategy[stratIndex].flopMDFSB.rangeArray[handIndex] > 0)) {
		// return false;
		// }
		// doRaise(87, POSTFLOP_MDF);
		// ++EvalData.mdfRaiseCountFlop[EvalData.seat];
	...
	private static boolean playHandMDFFlopBet1() {
		...
		// EvalData.decisionProcessRule[EvalData.streetNumber][EvalData.seat][EvalData.orbit]
		// = EvalData.DEC_MDFBET1;

		// if (EvalData.seat == seatButton &&
		// EvalData.strategy[stratIndex].flopMDFBet2Button.rangeArray[handIndex] > 0) {
		// doRaise(55, POSTFLOP_MDFBET2);
		// ++EvalData.mdfRaiseBet2CountFlop[EvalData.seat];
		// }
		// if (EvalData.seat == seatCO &&
		// EvalData.strategy[stratIndex].flopMDFBet2CO.rangeArray[handIndex] > 0) {
		// doRaise(55, POSTFLOP_MDFBET2);
		// ++EvalData.mdfRaiseBet2CountFlop[EvalData.seat];
		// return true;
		// }
