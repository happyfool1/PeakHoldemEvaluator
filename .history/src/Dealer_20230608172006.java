	...
	*********************************************************************************/
	...
	static void dealAllHoleCards() {
		for (int i = 0; i < PLAYERS; i++) {
			...
			if (EvalData.holeCards[i][0] == null || EvalData.holeCards[i][1] == null) {
				System.out.println("ERROR: Hole cards null card " + i + " " + EvalData.holeCards[i][0]
						+ " " + EvalData.holeCards[i][1]);
			}
			sortHoleCards(i);
...
print	static void dealFlop() {
		...
		if (EvalData.flop[0] == null || EvalData.flop[1] == null || EvalData.flop[2] == null) {
			Logger.logError("ERROR: Flop null card " + EvalData.flop[0]
		...
		if (EvalData.flop[0] == EvalData.flop[1] && EvalData.flop[0] == EvalData.flop[2]) {
			System.out.println("//AAA " + EvalData.board[0].value
	...
	/*- ******************************************************************************
	 * Sort the Flop, copy cards, set board values, evaluate board, Classify Flop
	 ********************************************************************************/
	static void flopGlobalValues() {
		...
		if (EvalData.manyHands) {
			Classification.doFlop();
		}
	}

	/*- ******************************************************************************
	 Copy turn card, Classify Turn
	 ********************************************************************************/
	static void turnGlobalValues() {
	...
	static void boardGlobalValues() {
		...
		EvalData.boardTwoPairValue1 = -1;
		EvalData.boardTwoPairValue2 = -1;
		EvalData.boardSet = false;
		EvalData.boardSetValue = -1;
		EvalData.boardHighCard = false;
		EvalData.boardHighCardValue = -1;
		EvalData.boardSet = false;
		EvalData.boardHighCard = false;
