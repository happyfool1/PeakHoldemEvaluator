...
// TODO Add color description - specifically RED
...
class EditRange implements Constants {
	...
	private void doFrame() {
		...
		for (int i = 0; i < HANDS; ++i) {
			buttonArr[i] = new JButton(range1.getRangeArrayString(i));
			if (!edit) {
	...
	private void editOne(String s0) {
		if (!"Average".equals(type)) {
			final int i = range1.getHandStringToIndex(s0);
			if (!subset) {
				if (buttonArr[i].getBackground() == range1Color) {
					buttonArr[i].setBackground(Color.GRAY);
					range1.setRangeArray(i, range1.NOT_RANGE);
				} else {
					buttonArr[i].setBackground(range1Color);
					range1.setRangeArray(i, range1.IN_RANGE);
				}
			} else {
				if (buttonArr[i].getBackground() == range1Color) {
					buttonArr[i].setBackground(Color.GRAY);
