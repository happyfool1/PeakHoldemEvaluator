		...
		for (int i = 0; i < HML_FLOP_SIZE; ++i) {
			...
			for (int j = 0; j < DRAW_HANDS_SIZE; ++j) {
				if (ManyData.hhmlDraw[i][j] > 0) {
					table3.setValueAt("   " + String.valueOf(ManyDatahhmlDraw[i][j]), row, col);
		...
		for (int i = 0; i < HML_FLOP_SIZE; ++i) {
			...
			for (int j = 0; j < MADE_HANDS_SIZE; ++j) {
				if (ManyData.hmlMade[i][j] > 0) {
					...
					double per = ((double) ManyData.hhmlMade[i][j] / (double) ManyDatahhmlCount) * 100.0;
...
********************************************************************************** */
	static void reportHMLShowdownFlop(int c, int r) {
		if (frame7 == null) {
			...
			tableModel7 = new DefaultTableModel(dataHHHMLMade, columnsHHHMLMade);
