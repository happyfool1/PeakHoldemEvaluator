...
public class Player implements Constants {
	...
	private void doFold() {
		...
		System.out.println("//doFold() seat " + this.seat);
	}
	...
	private void doBet3() {
		...
		canRaiseOrCall();
		if (canRaise) {
			doRaises(EvalData.betNowBD.multiply(EvalData.preflopBetMultiplyerOpenBD));
	...
	private void doBet4() {
		...
		canRaiseOrCall();
		if (canRaise) {
			doRaises(EvalData.betNowBD.multiply(EvalData.preflopBetMultiplyer4BetD));
		} else {
			doFold();
		}
	}
	...
	private void doAllinCall() {
		...
		System.out.println(
				"//doAllinCall() seat " + this.seat + " call " + call + " stack " + EvalData.stackBD[this.seat] + " pot " +
						EvalData.potBD);
	}
	...
	private void doCalls() {
		...
		System.out.println(
				"//doCalls() seat " + this.seat + " call " + call + " stack " + EvalData.stackBD[this.seat] + " pot " +
						EvalData.potBD);
	}
	...
	//	if (bet < Table.betNow) { // Rules require that bet must be same size or larger
	//		bet = Table.betNow;
	//	}
	//	if (bet - (int) bet != 0) { // if fractional part make no fraction
	//		bet += 1. - (bet - (int) bet);
	//	}
		...
		EvalData.betBD[0][this.seat][EvalData.orbit] = temp;
		EvalData.raisedToBD[0][this.seat][EvalData.orbit] = temp;
		EvalData.betNowBD = temp;
		//doRaises() seat 3 bet 6 temp 8.00 stack 194 pot 11.00

		//doR//doRaises() seat 3 bet 6 temp 8.00 stack 192.00 pot 11.00
		//raises() seat 5 bet 5.500 temp 13.500 stack 186.500 pot 24.500
