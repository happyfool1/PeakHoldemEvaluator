	...
	private static void doStreetHH(int street) {
		...
		for (int orbit = 0; orbit <= EvalData.lastOrbit[street]; ++orbit) {
			...
			for (int j = 0; j < seatCount; ++j) {
				...
				System.out.println("//HHH " + j + " returnedToBD " + EvalData.returnedToBD[street][seat][orbit] 
				+ " winBD " + EvalData.winBD[street][seat][orbit]);
				...
				if (EvalData.winBD[street][seat][orbit].compareTo(zeroBD) != 0) {
					...
					reportSt[lines++] += "\r\n";
					reportSt[lines] = nameStringArray[seat] + ": shows hand" + EvalData.holeCards[seat][0].toString() +EvalData.holeCards[seat][0].toString()  ;
					reportSt[lines++] += "\r\n";
				}
