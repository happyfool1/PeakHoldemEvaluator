...
public class EvaluateMany implements Constants {
	...
	static void updateShowdown() {
		for (int i = 0; i < PLAYERS; i++) {
			if (!EvalData.playerFolded[i] && EvalData.showdownWinnerHand[i] > 0) {
				++ManyData.hmlShowdownMase3[EvalData.hmlIndexFlop][EvalData.showdownWinnerHand[i]];
				++ManyData.hmlShowdownWin[EvalData.hmlIndexFlop][EvalData.showdownWinnerHand[i]];
				++ManyData.showdownCount;
				System.out.println("Showdown: " + EvalData.showdownWinnerHand[i]);
			}
		}
	}
