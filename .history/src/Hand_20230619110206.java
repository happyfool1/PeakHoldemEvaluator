	...
	*
	...
	private void finish(int type) {
		...
		if (this.size >= 4) {
			...
			if ( kicker1 == -1 || kicker2 == -1 || kicker3 == -1){
				System.out.println("EEE" + size + " " + cards[0] + cards[1] + cards[2] + cards[3] + cards[4] + " " +EvalData.both[0] +EvalData.both[1] 
			+EvalData.both[2]
			+EvalData.both[3] +  EvalData.both[4]);
			}
		} else {
			System.out.println("DDD " + size + " " + cards[0] + cards[1] + cards[2] + cards[3] + cards[4] + " " +EvalData.both[0] +EvalData.both[1] 
			+EvalData.both[2]
			+EvalData.both[3] +  EvalData.both[4]);
			if (EvalData.street == FLOP) {
