...
public class HoleCardAction implements Constants {
    ...
    *  Construct panel. 
    ...
    void constructPanel() {
        ...
        for (int i = 0; i < POSITIONS; ++i) {
            bet4CallText[i][0] = new JTextField(String.valueOf(actionArrayPlayabilityCall[i][2]));
            bet4CallText[i][0].setFont(f0);
            bet4CallText[i][0].setSize(40, 40);
            panelBet4Call.add(bet4CallText[i][0]);
        }
        for (int i = 0; i < POSITIONS; ++i) {
            bet4CallText[i][1] = new JTextField(Format.formatPer(actionArrayPercentageCall[i][2]));
