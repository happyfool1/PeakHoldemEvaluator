...
public class HoleCardAction implements Constants {
    ...
    *  Construct panel. 
    ...
    void constructPanel() {
     ...
     final var actionLayout = new GridLayout(4, 6);
     panelOpen.setLayout(actionLayout);
     panelOpen.setBorder(bet3Call);
     for (int i = 0; i < POSITIONS; ++i) {
         bet3CallText[i][0] = new JTextField(String.valueOf(actionArrayPlayabilityRaise[i][0]));
         bet3CallText[i][0].setFont(f0);
         bet3CallText[i][0].setSize(40, 40);
         panelOpen.add(bet3CallText[i][0]);
     }
     for (int i = 0; i < POSITIONS; ++i) {
           bet3CallText[i][1] = new JTextField(Format.formatPer(actionArrayPercentageRaise[i][0] ));
         bet3CallText[i][1].setFont(f0);
         bet3CallText[i][1].setSize(40, 40);
         panelOpen.add(bet3CallText[i][1]);
     }
     for (int i = 0; i < POSITIONS; ++i) {
         bet3CallText[i][2] = new JTextField(String.valueOf(actionArrayCombosRaise[i][0]));
         bet3CallText[i][2].setFont(f0);
         bet3CallText[i][2].setSize(40, 40);
         panelOpen.add(bet3CallText[i][2]);
     }
     for (int i = 0; i < POSITIONS; ++i) {
         ...
         panelOpen.add(bet3CallText[i][3]);
     }
    panel1.add(panelOpen);
    JPanel panelOpen = new JPanel();
    panelOpen.setSize(500, 80);
