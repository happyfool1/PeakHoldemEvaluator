...
public class Hand implements Constants {
	...
	* 
	...
	private void finish(int type) {
		...
		if (this.ok && this.size >= 4) {
			...
			if (EvalData.street == RIVER && this.type == MADE_FLUSH) {
				System.out.println("Hand " +	EvalData.highCards1[this.seat] + " " + EvalData.highCards2[this.seat]
				 + " " + EvalData.suits[this.seat] + " " + EvalData.kickers1[this.seat]);
			}
		}
		holeCardsFirst();
	}
	...
	* Is this fill house
	...
	boolean isFlush() {
		...
		if (this.ok) {
			System.out.println(isFlush() " " + EvalData.highCard1 + " " + EvalData.suit);
