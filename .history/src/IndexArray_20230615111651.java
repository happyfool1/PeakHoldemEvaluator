...
class PotOddsCalculator {
    ...
    public boolean isCallProfitable(double winningProbability) {
        // If the probability of winning is higher than the pot odds, then the call is profitable.
...
Sure, let's adjust the strategy based on the pot odds, similar to the way we did for stack size. Here's how you can modify the getStrategy method to take into account the pot odds:
...
This example adds a new potOdds field to the PlayerContext class and adjusts the strategy percentages in the getStrategy method based on the value of potOdds. If the pot odds are low (meaning the player has to risk a lot to win a little), the code increases the likelihood of folding or bluffing and decreases the likelihood of calling or raising. Conversely, if the pot odds are high (meaning the player can win a lot for a small risk), the code decreases the likelihood of folding or bluffing and increases the likelihood of calling or raising.
     */
    public StrategyPercentages getStrategy(FlopType flopType, HandType handType, PlayerContext context) {
        StrategyPercentages strategyPercentages = strategyMatrix.get(flopType).get(handType);
