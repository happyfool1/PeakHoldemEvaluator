...
public class Temp2 implements Constants {
	...
	* Sorted high to low, not in the order dealt but equivalents
	...
	/*- - Hole cards for each player PLAYERS hand. */
	 String[] hands = new String[PLAYERS]; // TODO
	 Card[] flop = new Card[3];
	 Card[] board = new Card[5];
	 Card[] boardUnsorted = new Card[5];
	...
	// Hole cards plus board cards sorted
	 final Card[] both = new Card[7];
	 bothCount = -1;
	// Copied only in manyHands is true
	 final Card[] flopHand = new Card[5];
	 final Card[] turnHand = new Card[6];
	// Sorted by card ( 0 - 51 ) for Flush and Flush draws
	 final Card[] boardCards = new Card[5];
	 final Card[] bothCards = new Card[7];
	/*-  *********************************************************************************** 
	 * 
	************************************************************************************** */
	 String[] playerNames = { "Hero", "Happy", "Fool", "JustMe", "Sue", "Chucky" };
	 double[] oddsForPlayerFlopToTurn = new double[PLAYERS];
	 double[] oddsForPlayerTurnToRiver = new double[PLAYERS];
	 double[] oddsForPlayerFlopToRiver = new double[PLAYERS];
	...
	playerPositions = new int[PLAYERS]; // indexed by position returns seat
	 ...
	 final int[] positionsStart = { SB, BB, UTG, MP, CO, BU };
	 ...
	 * positions is an array of SB BB UTG MP CO BU
	  ************************************************************************************** */
	 BigDecimal potBD = zeroBD;
	//  BigDecimal sidePotBD = zeroBD;
	 BigDecimal mainPotBD = zeroBD;
	//  BigDecimal potStartBD = zeroBD;
	//  BigDecimal potEndBD = zeroBD;
	 BigDecimal betToMeBD = zeroBD;
	 BigDecimal betNowBD = zeroBD;

	 BigDecimal[] stackBD = new BigDecimal[PLAYERS];
	 BigDecimal[] playerMainPotBD = new BigDecimal[PLAYERS];
	 double[] potOdds = new double[PLAYERS];

	relativePosition = new int[PLAYERS];
	 boolean[] playerAllin = new boolean[PLAYERS];
	[][] foldedPreflop = new int[4][PLAYERS];
	 BigDecimal[] stackPreflopBD = new BigDecimal[PLAYERS];
	 boolean[] playerFoldedPreflop = new boolean[PLAYERS];
	 boolean[] playerLimpedPreflop = new boolean[PLAYERS];
	 boolean[] playerFoldedFlop = new boolean[PLAYERS];
	 boolean[] playerFoldedTurn = new boolean[PLAYERS];
	 boolean[] playerFoldedRiver = new boolean[PLAYERS];
	 boolean[] playerWonShowdown = new boolean[PLAYERS];
	 BigDecimal[] moneyInBD = new BigDecimal[PLAYERS];
	 boolean[] playerWon = new boolean[PLAYERS]; // Indexed by street,seat,orbit
	...
	lastOrbit = new int[4]; // Indexed by street
	 boolean[][][] isFold = new boolean[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 boolean[][][] isLimp = new boolean[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 boolean[][][] isCheck = new boolean[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 boolean[][][] isCall = new boolean[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 boolean[][][] isBet = new boolean[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 boolean[][][] isRaise = new boolean[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 boolean[][][] isAllin = new boolean[4][PLAYERS][4]; // Indexed by street,seat,orbit

	 BigDecimal[][][] raisedToBD = new BigDecimal[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 BigDecimal[][][] returnedToBD = new BigDecimal[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 BigDecimal[][][] betBD = new BigDecimal[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 BigDecimal[][][] callBD = new BigDecimal[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 BigDecimal[][][] winBD = new BigDecimal[4][PLAYERS][4]; // Indexed by street,seat,orbit
	 BigDecimal[] winnerCollectedBD = new BigDecimal[PLAYERS]; // Indexed by street,seat,orbit
	 BigDecimal[] returnedToShowdownBD = new BigDecimal[PLAYERS]; // Indexed by street,seat,orbit
	 BigDecimal[] playerSidePotSplitTotalBD = new BigDecimal[PLAYERS]; // Indexed by street,seat,orbit
	 BigDecimal[] playerSidePotSplitBD = new BigDecimal[PLAYERS]; // Indexed by street,seat,orbit
	...
	*  but keep in mind that poker is a dynamic game that requires constant adjustments
	 ...
	 * 
	 ************************************************************************************** */
	 final BigDecimal SBBetBD = new BigDecimal(1);
	 final BigDecimal BBBetBD = new BigDecimal(2);
	 final BigDecimal preflopBetMultiplyerOpenBD = new BigDecimal(3);; // Times BB + 1 each limp
	 final BigDecimal preflopBetMultiplyer3BetBD = new BigDecimal(3);; // Times Open 2 Bet
	 final BigDecimal preflopBetMultiplyer4BetBD = new BigDecimal(2.5); // Times 3 Bet
	 final BigDecimal postFlopBbetMultiplierBD = new BigDecimal(.5); // Times pot size
	 final BigDecimal buyinBD = new BigDecimal(200);
	 final BigDecimal rebuyBD = new BigDecimal(100);
	 final BigDecimal maxStackBD = new BigDecimal(500);
	/*-  ************************************************************************************* 
	 * For the hand currently being evaluated
	* Results of evaluation 
	  ************************************************************************************** */
	// Flop board analysis - hole cards not included
	 boolean[] boardArray = new boolean[BOARD_ARRAY_SIZE]; // Board
	 boardArrayMax = -1;
	// Flop board analysis - hole cards included
	//  boolean[] drawAndMadeArray = new boolean[HANDS_SIZE];
	// Flop only information, not draw or made hand, just information
	 boolean[] flopArray = new boolean[FLOP_SIZE];
	// Drawing hands
	 boolean[] drawArray = new boolean[DRAW_HANDS_SIZE];
	// Made hands
	 boolean[] madeArray = new boolean[MADE_HANDS_SIZE];
	 madeArrayMax = -1;
	/*-  ************************************************************************************* 
	 ...
	 * and new information will continue to be revealed with each subsequent community card.
	  ************************************************************************************** */
	 handStrength = -1;
	 double rangeAdvantage = -1;
	 madeHand = -1;
	 bestDraw = -1;
	 double bestPossible = -1;
	 outs = -1;
	 double makeHandPer = -1;
	 flopTexture = -1;
	 turnTexture = -1;
	/*-  ************************************************************************************* 
	...
	************************************************************************************* */
	 boolean[] boardPossibleArrayFlop = new boolean[POSSIBLE_DRAW_SIZE];
	 boolean[] boardPossibleArrayTurn = new boolean[POSSIBLE_DRAW_SIZE];
	...
	***************************************************************************** */
	 boolean[] playerFolded = new boolean[PLAYERS];
	...
	*******************************************************************************/
	...
	flop3Suited = false;
	 ...
	 handsToPlay = 0;
	interactive = false;
	 String[] decSt = { "Rules", "MDFCheck", "MDFBet1", "MDFBet2", "Bluff", "ABC", "C-Bet", "GTO", "Barrel",
			"Final", "None" };
	 String[] rpSt = { "First", "First HU", "Middle", "Last", "Last HU" };
	 String[] actionSt = { "Check", "Bet", "Raise", "Call", "All-in", "Call All-in", "Fold", "None" };
	 String[] ruleSt = { "Bet", "Call", "All-in", "None" };
	 String[] opActionSt = { "Check", "Bet", "Raise", "Call", "All-in", "Call All-in", "Fold", "None" };
	 String[] streetSt = { "Preflop", "Flop", "Turn", "River", "None" };
	action = new int[PLAYERS]; // Range used
	 String releaseDate = "202100511";
	 String installDate = "202100511";
	 String expirationDateFree = "202100511";
	 String expirationDateTrial = "202100511";
	 String machineID = "chucky";
	 String UUID = "11324";
	 String version = "02.00.00";
	freeVersion = false;
	trialVersion = false;
	paidVersion = true;
