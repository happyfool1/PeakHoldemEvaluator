...
public class ReportHML implements Constants {
 ...
 private static final int DRAW_COL= DRAW_SIZE;
    private static final int DRAW_TOTAL_COL = MADE_FLUSH+2;
    ...
    ****************************************************************************************************************/
    static void reportDrawFlop(int c, int r) {
        ...
        for (int i = 0; i < HML_FLOP_SIZE; ++i) {
            ...
            for (int j = 0; j < DRAW_COL; ++j) {
                if (EvalDataMany.hmlDrawFlop[i][j] > 0) {
                    double per = ((double) EvalDataMany.hmlDrawFlop[i][j] / (double) EvalDataMany.hmlCountFlop) * 100.0;
                    sum += per;
                    sumx += per;
                    table1.setValueAt(Format.formatPer(per), row, col);
                    table1.setValueAt(Format.formatPer(sum), row, DRAW_TOTAL_COL);
                    String cellValueStr = "   " + String.valueOf(EvalDataMany.hmlDrawFlop[i][j]);
        ...
        for (int i = 1; i < DRAW_COL; ++i) {
          double  per = ((double) EvalDataMany.hmlDrawColTotalFlop[i] / (double) EvalDataMany.hmlCountFlop) * 100.0;
                 table1.setValueAt(Format.formatPer(per), row, i+1);
        }
        System.out.println("draw sumx " + sumx);
    ...
    static void reportMadeFlop(int c, int r) {
        ...
        for (int i = 0; i < HML_FLOP_SIZE; ++i) {
            ...
            for (int j = 0; j < MADE_COL; ++j) {
                if (EvalDataMany.hmlMadeFlop[i][j] > 0) {
                      double per = ((double) EvalDataMany.hmlMadeFlop[i][j] / (double) EvalDataMany.hmlCountFlop) * 100.0;
                    sum += per;
                    sumx += per;
                    table2.setValueAt(Format.formatPer(per), row, col);
                    table2.setValueAt(Format.formatPer(sum), row,  MADE_TOTAL_COL);
                    String cellValueStr = "   " + String.valueOf(EvalDataMany.hmlMadeFlop[i][j]);
                    // Parse the cell value to an integer and add to the priority queue
                    if (col > 1) {
                        try {
                            int cellValue = Integer.parseInt(cellValueStr.trim());
    ...
    static void reportShowdownFlop(int c, int r) {
        ...
        for (int i = 0; i < HML_FLOP_SIZE; ++i) {
            ...
            for (int j = 0; j <  MADE_COL; ++j) {
                if (EvalDataMany.hmlShowdownFlop[i][j] > 0) {
                    double per = ((double) EvalDataMany.hmlShowdownFlop[i][j] / (double) EvalDataMany.showdownCount) * 100.0;
                    sum += per;
                    sumx += per;
                    table3.setValueAt(Format.formatPer(per), row, col);
                    table3.setValueAt(Format.formatPer(sum), row,  MADE_TOTAL_COL);
                    String cellValueStr = "   " + String.valueOf(EvalDataMany.hmlShowdownFlop[i][j]);
                    // Parse the cell value to an integer and add to the priority queue
    ...
    static void reportDrawTurn(int c, int r) {
        ...
        for (int i = 0; i < HML_TURN_SIZE; ++i) {
            ...
            sum = 0.0;
            for (int j = 0; j < DRAW_COL; ++j) {
                if (EvalDataMany.hmlDrawTurn[i][j] > 0) {
                    double per = ((double) EvalDataMany.hmlDrawTurn[i][j] / (double) EvalDataMany.hmlCountTurn) * 100.0;
                    sum += per;
                    sumx += per;
                    table4.setValueAt(Format.formatPer(per), row, col);
                    table4.setValueAt(Format.formatPer(sum), row, DRAW_TOTAL_COL);
                    String cellValueStr = "   " + String.valueOf(EvalDataMany.hmlDrawTurn[i][j]);
                    // Parse the cell value to an integer and add to the priority queue
                    if (col > 1) {
                        try {
                            int cellValue = Integer.parseInt(cellValueStr.trim());
    ...
    static void reportMadeTurn(int c, int r) {
        ...
        for (int i = 0; i < HML_TURN_SIZE; ++i) {
            ...
            sum = 0.0;
            for (int j = 0; j < MADE_COL; ++j) {
                if (EvalDataMany.hmlMadeTurn[i][j] > 0) {
                    double per = ((double) EvalDataMany.hmlMadeTurn[i][j] / (double) EvalDataMany.hmlCountTurn) * 100.0;
                    sum += per;
                    sumx += per;
                    table5.setValueAt(Format.formatPer(per), row, col);
                    table5.setValueAt(Format.formatPer(sum), row, MADE_TOTAL_COL);
                    String cellValueStr = "   " + String.valueOf(EvalDataMany.hmlMadeTurn[i][j]);
                    // Parse the cell value to an integer and add to the priority queue
                    if (col > 1) {
                        try {
                            int cellValue = Integer.parseInt(cellValueStr.trim());
    ...
    static void reportShowdownTurn(int c, int r) {
        ...
        for (int i = 0; i < HML_TURN_SIZE; ++i) {
            ...
            col = 1;
            for (int j = 0; j < MADE_COL; ++j) {
                if (EvalDataMany.hmlShowdownTurn[i][j] > 0) {
                    double per = ((double) EvalDataMany.hmlShowdownTurn[i][j] / (double) EvalDataMany.showdownCount) * 100.0;
                    sum += per;
                    sumx += per;
                    table6.setValueAt(Format.formatPer(per), row, col);
                    table6.setValueAt(Format.formatPer(sum), row, MADE_TOTAL_COL);
                    String cellValueStr = "   " + String.valueOf(EvalDataMany.hmlShowdownTurn[i][j]);
                    // Parse the cell value to an integer and add to the priority queue
                    if (col > 1) {
                        try {
                            int cellValue = Integer.parseInt(cellValueStr.trim());
    ...
    ********************************************************************************** */
    static void reportMadeRiver(int c, int r) {
        ...
        for (int i = 0; i < HML_RIVER_SIZE; ++i) {
            ...
            sum = 0.0;
            for (int j = 0; j < MADE_COL; ++j) {
                if (EvalDataMany.hmlMadeRiver[i][j] > 0) {
                    double per = ((double) EvalDataMany.hmlMadeRiver[i][j] / (double) EvalDataMany.hmlCountRiver) * 100.0;
                    sum += per;
                    sumx += per;
                    table7.setValueAt(Format.formatPer(per), row, col);
                    table7.setValueAt(Format.formatPer(sum), row, MADE_TOTAL_COL);
                    String cellValueStr = "   " + String.valueOf(EvalDataMany.hmlMadeRiver[i][j]);
                    // Parse the cell value to an integer and add to the priority queue
                    if (col > 1) {
                        try {
                            int cellValue = Integer.parseInt(cellValueStr.trim());
    ...
    ********************************************************************************** */
    static void reportShowdownRiver(int c, int r) {
        ...
        for (int i = 0; i < HML_RIVER_SIZE; ++i) {
            ...
            sum = 0.0;
            for (int j = 0; j < MADE_COL; ++j) {
                if (EvalDataMany.hmlShowdownRiver[i][j] > 0) {
                    double per = ((double) EvalDataMany.hmlShowdownRiver[i][j] / (double) EvalDataMany.showdownCount) * 100.0;
                    sum += per;
                    sumx += per;
                    table8.setValueAt(Format.formatPer(per), row, col);
                    table8.setValueAt(Format.formatPer(sum), row, MADE_TOTAL_COL);
                    String cellValueStr = "   " + String.valueOf(EvalDataMany.hmlShowdownRiver[i][j]);
                    // Parse the cell value to an integer and add to the priority queue
                    try {
                        int cellValue = Integer.parseInt(cellValueStr.trim());
                        // For smallest cells
