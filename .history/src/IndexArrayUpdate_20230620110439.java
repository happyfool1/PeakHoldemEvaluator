...
public class IndexArrayUpdate implements Constants {
	...
	for (int i = 0; i < PLAYERS; i++) {
			if (!EvalData.playerFolded[i] && EvalData.playerWonShowdown[i]) {
					...
					++IndexArrays.hmlMadeTurnToMadeWon[EvalData.hmlIndexTurn]  [EvalData.madeTypeTurni]   [EvalData.showdownHand[i]];
					++IndexArrays.hmlDrawTurnToMadeWon[[EvalData.hmlIndexTurn]  [EvalData.draweTypeTurn[i]  [EvalData.showdownHand[i]];
