...
public class IndexArrayUpdate implements Constants {
	...
	for (int i = 0; i < PLAYERS; i++) {
			if (!EvalData.playerFolded[i] && EvalData.playerWonShowdown[i]) {
					...
					++IndexArrays.hmlDrawFlopToMadeWon[EvalData.hmlIndexFlop[ [EvalData.madeTypeFlop[i] [EvalData.showdownHand[i]];

					++IndexArrays.hmlMadeTurnToMadeWon[EvalData.hmlIndexTurn]  [EvalData.madeTypeFlop[i]   [EvalData.showdownHand[i]];
					++IndexArrays.hmlDrawTurnToMadeWon[[EvalData.hmlIndexTurn]  [EvalData.madeTypeFlop[i]  [EvalData.showdownHand[i]];
