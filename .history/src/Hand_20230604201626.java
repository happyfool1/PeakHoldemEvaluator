...
public class Hand implements Constants {
	...
	* }
	* 
Top Pair: This refers to having a pair with the highest card on the flop. For example, if you have AK and the flop comes A-7-2, you have the top pair (a pair of Aces).

Middle Pair: This refers to having a pair with the middle card on the flop. For example, if you have 7K and the flop comes A-7-2, you have the middle pair (a pair of Sevens).

Bottom Pair: This refers to having a pair with the lowest card on the flop. For example, if you have 2K and the flop comes A-7-2, you have the bottom pair (a pair of Twos).

Over Pair: This refers to having a pair in your hand that is higher than any card on the board. For example, if you have QQ and the flop comes J-7-2, you have an over pair (a pair of Queens, which is high
er than any card on the board).

The strength of your pair can greatly influence your decisions in poker. A top pair or over pair is generally a strong hand, while a middle or bottom pair can often be vulnerable. However, context is crucial: the strength of your hand should always be evaluated relative to what other hands your opponents might have.
...
ChatGPT
Yes, when a player has two pair, they're often described based on the cards that make up the pairs. While not as commonly referred to as top pair, middle pair, bottom pair, or over pair, these classifications can still be helpful in analyzing the strength of the hand. Here are a few possible scenarios:

Top Two Pair: When a player's hand includes the two highest cards on the board. For example, if you hold A-7 and the flop is A-7-2, you have top two pair.

Top and Bottom Pair: When a player's hand includes the highest and the lowest card on the board. For example, if you hold A-2 and the flop is A-7-2, you have top and bottom pair.

Bottom Two Pair: When a player's hand includes the two lowest cards on the board. For example, if you hold 7-2 and the flop is A-7-2, you have bottom two pair.

Just like with a single pair, the strength of your two pair hand can greatly influence your decisions in poker. In general, two pair is a strong hand, but the exact strength of it can vary significantly based on the board texture, possible hands of your opponents, and your position at the table. Always consider these factors when deciding how to proceed with your hand.
	...
	* Not conclusive for Turn or River because it is possible that a hole card was not
	* included in boolean result.
	* Returns MADE_TOP+PAIR, MADE_MIDDLE_PAIR, MADE_BOTTON_PAIR, or MADE_OVER_PAIR
	/*- ************************************************************************************/
	int whatKindOfPair() {
		// Find high card on flop that is paired
		int highCard = -1;
		if (this.size == 5) {
	...
	* Returns MADE_TOP+PAIR, MADE_MIDDLE_PAIR, MADE_BOTTON_PAIR, or MADE_OVER_PAIR
	 ...
	 *		EvalData.highCard1  
	 *		EvalData.highCard2  		EvalData.highCard1  
	 ***************************************************************************************/
	boolean isTwoPair() {
		initialize();
		addCardsOfPair();
