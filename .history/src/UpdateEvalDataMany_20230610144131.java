...
public class UpdateEvalDataMany implements Constants {
	...
	static void updateFlop() {
		...
		for (int i = DRAW_SIZE - 1; i >= 0; i--) {
			if (EvalData.drawArray[i]) {
				EvalDataMany.hmlDraw[index][i]++;
				break;
			}
		}
		for (int i = MADE_SIZE - 1; i >= 0; i--) {
			if (EvalData.madeArray[i]) {
				EvalDataMany.hmlMade[index][i]++;
				if (i == MADE_BOARD_SET) {
	...
	static void updateTurn() {
		++EvalDataMany.hhmlCount;
		++EvalDataMany.turnArraysCount;

		int index = EvalData.hmlIndexTurn;
		for (int i = DRAW_SIZE - 1; i >= 0; i--) {
			if (EvalData.drawArray[i]) {
				++EvalDataMany.hhmlDraw[index][i];
				break;
			}
		}
		for (int i = MADE_SIZE - 1; i >= 0; i--) {
			if (EvalData.madeArray[i]) {
				++EvalDataMany.hhmlMade[index][i];
				break;
			}
		}
	}
		...
		for (int i = 0; i < PLAYERS; i++) {
			if (!EvalData.playerFolded[i] && EvalData.playerWonShowdown[i]) {
				if (EvalData.hmlIndexFlop != -1) {
					++EvalDataMany.hmlShowdownWon[EvalData.hmlIndexFlop][EvalData.showdownHand[i]];
				}
				if (EvalData.hmlIndexTurn != -1) {
					++EvalDataMany.hhmlShowdownWon[EvalData.hmlIndexTurn][EvalData.showdownHand[i]];
				}
				if (EvalData.hmlIndexRiver != -1) {
					++EvalDataMany.hhhmlShowdownWon[EvalData.hmlIndexRiver][EvalData.showdownHand[i]];
				}
	...
	static void updateSummary() {
		for (int i = 0; i < HML_FLOP_SIZE; i++) {
			for (int j = 0; j < DRAW_SIZE; j++) {
				EvalDataMany.hhmlDrawRowTotal[i] += EvalDataMany.hhmlDraw[i][j];
				EvalDataMany.hmlDrawSumOfAllValues += EvalDataMany.hhmlDraw[i][j];
			}
		}
		for (int j = 0; j < DRAW_SIZE; j++) {
			for (int i = 0; i < HML_FLOP_SIZE; i++) {
				EvalDataMany.hhmlDrawColTotal[i] += EvalDataMany.hhmlDraw[i][j];
			}
		}
		for (int i = 0; i < HML_FLOP_SIZE; i++) {
			for (int j = 0; j < MADE_SIZE; j++) {
				EvalDataMany.hhmlMadeRowTotal[i] += EvalDataMany.hhmlMade[i][j];
				EvalDataMany.hmlMadeSumOfAllValues += EvalDataMany.hhmlMade[i][j];
			}
		}
		for (int j = 0; j < MADE_SIZE; j++) {
			for (int i = 0; i < HML_FLOP_SIZE; i++) {
				EvalDataMany.hhmlMadeColTotal[i] += EvalDataMany.hhmlMade[i][j];
						}
		}
		for (int i = 0; i < HML_FLOP_SIZE; i++) {
			for (int j = 0; j < MADE_SIZE; j++) {
				EvalDataMany.hhmlShowdownRowTotal[i] += EvalDataMany.hhmlShowdownWon[i][j];
				EvalDataMany.hmlShowdownSumOfAllValues += EvalDataMany.hhmlShowdownWon[i][j];
			}
		}
		for (int j = 0; j < MADE_SIZE; j++) {
			for (int i = 0; i < HML_FLOP_SIZE; i++) {
				EvalDataMany.hhmlShowdownColTotal[i] += EvalDataMany.hhmlShowdownWon[i][j];
			}
		}
