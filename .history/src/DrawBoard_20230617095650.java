...
public class DrawBoard implements Constants {
	...
	boolean isSet() {
		...
		if (this.ok) {
			if (EvalData.street == FLOP) {
				EvalData.drawTypeFlop[EvalData.seat] = MADE_FOUR_OF_A_KIND_SET;
			}
			if (EvalData.street == TURN) {
	...
	boolean isFlush() {
		...
		if (this.ok) {
			if (EvalData.street == FLOP) {
				EvalData.madeTypeFlop[EvalData.seat] = MADE_FLUSH;
			}
			if (EvalData.street == TURN) {
				EvalData.madeTypeTurn[EvalData.seat] = MADE_FLUSH;
			} else {
				EvalData.madeTypeRiver[EvalData.seat] = MADE_FLUSH;
			}
	...
	boolean isStraight() {
		...
		if (this.ok) {
			if (EvalData.street == FLOP) {
				EvalData.madeTypeFlop[EvalData.seat] = MADE_STRAIGHT;
			}
			if (EvalData.street == TURN) {
				EvalData.madeTypeTurn[EvalData.seat] = MADE_STRAIGHT;
			} else {
				EvalData.madeTypeRiver[EvalData.seat] = MADE_STRAIGHT;
			}
			EvalData.highCard1 = EvalData.bothCards[0].value;
		}
		return this.ok;
