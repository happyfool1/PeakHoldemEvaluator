...
public class DrawBoard implements Constants {
	...
	boolean isFullHouse() {
		...
		if (this.ok) {
			if (EvalData.street == FLOP) {
				...
				EvalData.drawTypeTurn[EvalData.seat] = MADE_FULL_HOUSE;
				} else {
                    EvalData.drawTypeRiver[EvalData.seat] = MADE_FULL_HOUSE;

                            }
                            if (EvalData.street == TURN) {
                                EvalData.drawTypeTurn[EvalData.seat] = DRAW_FULL_HOUSE;
                            } else {
                                EvalData.drawTypeRiver[EvalData.seat] = DRAW_FULL_HOUSE;
                            }
			EvalData.madeArray[MADE_FULL_HOUSE] = true;
		}
		return this.ok;
	}
	...
	boolean is4Of() {
		...
		if (this.ok) {
			EvalData.madeArray[MADE_FOUR_OF_A_KIND] = true;
		}
		return this.ok;
	}
