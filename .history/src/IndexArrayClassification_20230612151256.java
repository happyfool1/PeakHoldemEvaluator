...
public class IndexArrayClassification implements Constants {
 ...
 /*-*****************************************************************************
 * This method will classify a Flop.
 * Another approach to characterizing the flop can be by looking at the Number Distribution and Flush Potential.

Number Distribution: This categorizes the flop based on how many of each rank are present.

Trips: All three cards have the same rank.
One Pair: Two cards have the same rank, and the third card has a different rank.
No Pair: All three cards have different ranks.
Flush Potential: This categorizes the flop based on the suits of the cards, similar to Suitedness.

High: All three cards have the same suit, implying high potential for a flush.
Medium: Two cards have the same suit, but the third is of a different suit, indicating medium potential for a flush.
Low: All three cards have different suits, indicating low potential for a flush.
  ******************************************************************************/
	    public static int flopTextureDistributionFlushPotential( ) {
         // Determine number distribution type
        int numberDistributionType;
        if (EvalData.board[0].rank == EvalData.board[1].rank && EvalData.board[1].rank == EvalData.board[2].rank) {
            numberDistributionType = 1; // Trips
        } else if (EvalData.board[0].rank == EvalData.board[1].rank || EvalData.board[1].rank == EvalData.board[2].rank || EvalData.board[0].rank == EvalData.board[2].rank) {
        ...
        // Return the combined characterization as an integer
        return (numberDistributionType - 1) * 3 + flushPotentialType;
    }
