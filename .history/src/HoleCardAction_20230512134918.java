...
public class HoleCardAction implements Constants {
    ...
    *  Construct panel. 
    ...
    void constructPanel() {
        ...
        Border bet3Call = BorderFactory.createTitledBorder("3Call     Playability, Percentage, Combos, Gaps");
        ...
        panelCall.setBorder(bet3Call);
        for (int i = 0; i < POSITIONS; ++i) {
            bet3CallText[i][0] = new JTextField(String.valueOf(actionArrayPlayabilityCall[i][1]));
            bet3CallText[i][0].setFont(f0);
            bet3CallText[i][0].setSize(40, 40);
            panelCall.add(callText[i][0]);
        }
        for (int i = 0; i < POSITIONS; ++i) {
            bet3CallText[i][1] = new JTextField(Format.formatPer(actionArrayPercentageCall[i][1]));
            bet3CallText[i][1].setFont(f0);
            bet3CallText[i][1].setSize(40, 40);
            panelCall.add(callText[i][1]);
        }
        for (int i = 0; i < POSITIONS; ++i) {
            bet3CallText[i][2] = new JTextField(String.valueOf(actionArrayCombosCall[i][1]));
            bet3CallText[i][2].setFont(f0);
            bet3CallText[i][2].setSize(40, 40);
            panelCall.add(callText[i][2]);
        }
        for (int i = 0; i < POSITIONS; ++i) {
            bet3CallText[i][3] = new JTextField(String.valueOf(actionArrayGapsRaise[i][1]));
            bet3CallText[i][3].setFont(f0);
            bet3CallText[i][3].setSize(40, 40);
            panelCall.add(bet3CallText[i][3]);
        }
