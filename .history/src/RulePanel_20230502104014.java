...
public class RulePanel implements Constants {
	...
	public JPanel updatePanel(String path) {
		if (Table.streetNumber != PREFLOP) {
			...
			pos = "Middle";
			file = "Rule.ser";
			if (Table.streetNumber == FLOP) {
				street = "Flop";
			}
			if (Table.streetNumber == TURN) {
				street = "Turn";
			}
			if (Table.streetNumber == RIVER) {
				street = "River";
			}
			if (Play.first == 0) {
				if (Table.foldCount - PLAYERS == 2) {
			...
			handRule.setText(rules.playToString(rules.getPlay(handValue, Table.betType)));
