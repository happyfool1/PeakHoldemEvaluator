...
public class IndexArrayClassification implements Constants {
	...
	*****************************************************************************/
	private static void flopTexturC() {
        ...
        // Sort cards by rank
        Arrays.sort(cards, Comparator.comparing(card -> card.rank));

        // Determine suit type
        String suitType;
        if (cards[0].suit == cards[1].suit && cards[1].suit == cards[2].suit) {
            suitType = "Monotone";
        } else if (cards[0].suit == cards[1].suit || cards[1].suit == cards[2].suit || cards[0].suit == cards[2].suit) {
            suitType = "Two-tone";
        } else {
            suitType = "Rainbow";
        }
        ...
        if (cards[1].rank - cards[0].rank == 1 && cards[2].rank - cards[1].rank == 1) {
            connectivityType = "Connected";
        } else if (cards[1].rank - cards[0].rank == 1 || cards[2].rank - cards[1].rank == 1) {
