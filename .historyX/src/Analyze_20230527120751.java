...
class Analyze implements Constants {
	...
	static void evaluateFlop(int seat) {
		...
		Classification.boardPossiblesFlop();
		Classification.evaluateDraws();
		Classification.flopType();
		Classification.wetDry();
		Classification.boardIndexesFlop();
	...
	static void evaluateTurn(int seat) {
		...
		Classification.boardPossiblesTurn();
		Classification.evaluateDraws();
		Classification.hmlBoard();
		for (int i = POSSIBLE_MAX - 1; i >= 0; --i) {
	...
	***************************************************************************** */
	 ...
	 * 
	 * They are then used to create two more attack sorted by card valur ( 0 - 51 ) which places 
