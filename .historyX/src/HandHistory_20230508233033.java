	...
	private static void doStreetHH(int street) {
		...
		for (int orbit = 0; orbit <= EvalData.lastOrbit[street]; ++orbit) {
			...
			for (int j = 0; j < seatCount; ++j) {
				...
				System.out.println(new StringBuilder().append("//HHH ").append(j).append(" returnedToBD ")
						.append(EvalData.returnedToBD[street][seat][orbit].toString()).append(" winBD ")
						.append(EvalData.winBD[street][seat][orbit]).toString() + " " + seat + " " + orbit);
