...
public class HandRange implements Constants {
	...
	private static int COMBINATIONS = 1326;
	...
	String cards = ""; // Cards in HandRange value is handIndex
	...
	int findLast() {
		...
		for (int i = HANDS; i >= 0; i--) {
			if (this.rangeArray[i] >= 0) {
				lowRangeArrayIndex = i;
				lowPlayabilityArrayNumericValue = playabilityArrayNumeric[i]; 
				break;
			}
		}
		// Find lowest playabilityArray element with a hand
		for (int i = HANDS; i >= 0; i--) {
			if (this.orderArray[i] >= 0) {
				lowPlayabilitrhyArrayIndex = 
				break;
			}
		}
				this.gapCount = 0;
