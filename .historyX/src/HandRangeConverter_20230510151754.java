...
public class HandRangeConverter implements Constants {
	...
	private static void copyData() {
		...
		for (int i = 0; i < HANDS; i++){
			action.rangeArray[SB][i] = rangeSBAllinCallrangeArray[i];
			action.gapsArray[SB][i] = rangeSBAllinCallgapsArray[i];
		}
		action.cards[SB] = rangeSBAllinCallcards;
		action.lowPlayabilitrhyArrayIndex[SB] = rangeSBAllinCalllowPlayabilitrhyArrayIndex;
		action.lowRangeArrayIndex [SB] = rangeSBAllinCalllowRangeArrayIndex ;
		action.lowPlayabilityArrayNumericValue[SB] = rangeSBAllinCalllowPlayabilityArrayNumericValue;
		action.gapCount[SB] = rangeSBAllinCallgapCount;
