...
public class GUI extends JFrame implements ActionListener, Constants {
	...
	private static final JTextArea pot = new JTextArea("Pot");
	...
	private static final JButton step = new JButton("Step");
	...
	private static final JCheckBox freezeSeat = new JCheckBox("Freeze Seat");
	 ...
	 * Data set when a card in cardArrray  or in hole1Array or hole2Array or 
	...
	*******************************************************************************/
	...
	private boolean freezeSeatOption = false;

	private boolean detailedReportOption = false;

	private boolean boardReportOption = false;

	private final boolean preflopReportOption = false;
	...
	*******************************************************************************/
	public void myFrame() {
		...
		panelPotOdds = new JPanel(new GridLayout(1, 5));
		final var panelBSize = new Dimension(40, 40);
		panelPot.setMaximumSize(panelBSize);
		panelPot.setPreferredSize(panelBSize);
		panelPot.setBounds(600, 260, 250, 40);
		...
		panelControl = new JPanel(new GridLayout(1, 5));
		panelControl.setBounds(300, 420, 650, 40);
		...
		for (int i = 0; i < 52; i++) {
			...
			cardArray[i].addMouseListener(new DragListener());
			if (c.suit == 0) {
		...
		for (int i = 0; i < 6; i++) {
			...
			hole1Array[i].addMouseListener(new DragListener());
			...
			hole2Array[i].addMouseListener(new DragListener());
		...
		freezeSeat.setFont(fs);
		freezeSeat.setPreferredSize(new Dimension(25, 25));
		freezeSeat.addActionListener(this);
		panelCheckBoxesOptions.add(freezeSeat);
	...
	private void doExecute() {
		if (preflopSimulationOption) {
			...
			while (!done) {
				...
				--limit;
				if (EvalData.foldCount < 3 || limit == 0) {
					done = true;
				}
			}
			holeCardsFolded();
			System.out.println("//xxx  " + limit);
		}
	...
	/*-  ******************************************************************************
	 * The Step button has been clicked.
	 *  ****************************************************************************** */
	private void doStep() {
	...
	****************************************************************************** */
	...
	private void randomBoard() {
		...
		Evaluate.dealFlop();
		Evaluate.dealTurnCard();
		Evaluate.dealRiverCard();
		// Now remove these cards from card array
		for (int i = 0; i < 5; i++) {
	...
	private void checkBoxesl() {
		...
		if (freezeSeat.isSelected()) {
			freezeSeatOption = true;
		} else {
			freezeSeatOption = false;
		}
	...
	private void updatePlayability() {
		...
		for (int i = 0; i < 6; i++) {
			if (hole1Array[i].getText().length() == 2 && hole2Array[i].getText().length() == 2) {
				card1 = new Card(hole1Array[i].getText());
				card2 = new Card(hole2Array[i].getText());
				final int ndx = Evaluate.getArrayIndexCard(card1, card2);
	...
	* a radio button or control button.
	...
	/*-  ******************************************************************************
	 *  Here is where the application mostly takes place.
	 *  The cardArray has the 52 possible cards.
	 *  Any card can be dragged to another card button ( other than cardArray ) 
	 *   and placed there. 
	 *   The card in cardArray is set to null.
	 *   If there is a card in the new location it is removed and returned to cardArray
	 *   before the new card is placed there. 
	 *   All of the 52 cards are always somewhere in the JFrame.
	 *   
	 *   When we created the buttons we put them into a HashMap.
	 *   The key is unique text asociated with each button.
	 *   	For cardArray it is the card. Ex:  "As"
	 *   	For handArray it is "hxa" or "hxb" x is index
	 *   	For boardArray it is "box"  x is index
	 *   	For seat radio buttons it is "Seat1" ...
	 *   	For street radio buttons it is "Flop" ...   
	 *   
	 *   TODO
	 *   I have been unable to get dran and drop to work correctly so changed to the 
	 *   current method of clicking on source then clicking on destination.
	 *   When I have more time I will redue this using drag and drop.
	 *   What I have been unable to do is find the X and Y locations of a JButton,
	 *   button.getX() always returns 0.
	 *   On the  mouseReleased event the event has the source text, not the destination text.
	 *   This works to get a string than can be parsed. e is event
	 *  	String st = e.paramString();
	 *		int i1 = st.indexOf("absolute(");
	 *		int i2 = st.indexOf(",", i1 + 8);
	 *		String st2 = st.substring(i1 + 9, i2);
	 *		int i3 = st.indexOf(")", i2);
	 *		String st3 = st.substring(i2 + 1, i3);
	 * 
	 *  ****************************************************************************** */
	...
	private class DragListener extends MouseAdapter {
		...
		public void mousePressed(MouseEvent e) {
			...
			// System.out.println("//pressed " + draggedButton.getText());
		...
		public void mouseReleased(MouseEvent e) {
			...
			final int i3 = st.indexOf(")", i2);
					// System.out.println("//released " + button.getText() + "--" + st2 + "--" + st3
			// + "--");

			if (panelHoleCards.contains(e.getPoint())) {
			...
			/*-
				if (panelHoleCards.contains(e.getPoint())) {
					...
					panelHoleCards.add(draggedButton);
			
					// Remove the button from panel 1
					panelCards.remove(draggedButton);
			
					// Repaint the panels
					panelCards.repaint();
					panelHoleCards.repaint();
			
					// Update the button location in the buttonLocations map
					// buttonLocations.put(draggedButton,
					// new Point(panelCards.getX() + draggedButton.getX(), panelCards.getY() +
					// draggedButton.getY()));
				} else if (panelBoard.contains(e.getPoint())) {
					// Add the button to the text panel
					panelBoard.add(draggedButton);
					...
					// Update the button location in the buttonLocations map
					// buttonLocations.put(draggedButton,
					// new Point(textPanel.getX() + draggedButton.getX(), textPanel.getY() +
					// draggedButton.getY()));
				} else if (panelFlop.contains(e.getPoint())) {
					// Add the button to the text panel
					panelFlop.add(draggedButton);
					panelFlop.revalidate();
					panelFlop.repaint();
					...
					// new Point(textPanel.getX() + draggedButton.getX(), textPanel.getY() +
					// draggedButton.getY()));
				} else if (panelTurn.contains(e.getPoint())) {
					// Add the button to the text panel
					panelTurn.add(draggedButton);
					panelTurn.revalidate();
					panelTurn.repaint();
					...
					// Update the button location in the buttonLocations map
			//				buttonLocations.put(draggedButton,
			//						new Point(textPanel.getX() + draggedButton.getX(), textPanel.getY() + draggedButton.getY()));
				} else if (panelRiver.contains(e.getPoint())) {
					// Add the button to the text panel
					panelRiver.add(draggedButton);
					panelRiver.revalidate();
					panelRiver.repaint();
			...
			//						new Point(textPanel.getX() + draggedButton.getX(), textPanel.getY() + draggedButton.getY()));
					...
					// If the button was dropped on a location other than panel 2 or the text panel,
					// reset its location to the original location
					draggedButton.setLocation(buttonLocations.get(draggedButton));
				}
		...
		private class DragListenerX extends MouseAdapter {
			...
			public void mousePressed(MouseEvent e) {
				...
				/*-
						...
						// Get the offset between the mouse click and the top-left corner of the button
						offset = e.getPoint();
				
						// Create a new cursor for dragging
						draggingCursor = Cursor.getPredefinedCursor(Cursor.MOVE_CURSOR);
				
						// Set the cursor to the dragging cursor
						mainPanel.setCursor(draggingCursor);
			...
			public void mouseReleased(MouseEvent e) {
				// JButton button = (JButton) e.getSource();
				// System.out.println("//dragX " + button.getText());
				// System.out.println("//dragX " + e);
				/*-
				// Reset the cursor to the default cursor
				mainPanel.setCursor(Cursor.getDefaultCursor());
				
				// Check if the button was dropped on panel 2
				if (panelCards.contains(e.getPoint())) {
					// Add the button to panel 2
					panelCards.add(draggedButton);
					...
					// Update the button location in the buttonLocations map
				//				buttonLocations.put(draggedButton, new Point(panelCards.getX() + draggedButton.getX(),
				//							panelCards.getY() + draggedButton.getY()));
				} else {
					// If the button was dropped on a location other than panel 2, reset its
			...
			public void mouseDragged(MouseEvent e) {
				...
				// JButton button = (JButton) e.getSource();
				// System.out.println("//dragY " + button.getText());
				// System.out.println("//dragY " + e);
				/*-
				// Calculate the new location of the button based on the mouse position and the
				// offset
