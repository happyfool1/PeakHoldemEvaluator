...
public class GameControl implements Constants {
	...
	**************************************************************************** */
	...
	private static void playHandPreflop() {
		System.out.println(new StringBuilder().append("//playHand ").append(seat).append(" ").append(seatSB).append(" ")
				.append(seatBB).append(" ").append(seatUTG).append(" ").append(seatMP).append(" ").append(seatCO)
				.append(" ").append(seatBU).toString());
	...
	*  Return uncalled bet
	...
	static void returnUncalledBet() {
		...
		int s = -1;
		for (int i = 0; i < PLAYERS; i++) {
			System.out.println(new StringBuilder().append("//folded ").append(EvalData.playerFolded[i]).append(" ")
					.append(EvalData.foldCount).toString());
			if (!EvalData.playerFolded[i]) {
				s = i;
				break;
			}
