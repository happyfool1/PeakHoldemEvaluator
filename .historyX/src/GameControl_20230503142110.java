...
public class GameControl implements Constants {
	...
	*******************************************************************************/
	...
	static int seatBU = -1;
	private static final int foldCount = 0;
	...
	private static BigDecimal betToMe = zeroBD;
	private static BigDecimal betNow = zeroBD;
	...
	**************************************************************************** */
	...
	private static void initialize() {
		for (int i = 0; i < PLAYERS; i++) {
			EvalData.stackBD[i] = zeroBD;
			EvalData.potOdds[i] = 0;
			EvalData.relativePosition[i] = 0;
			EvalData.moneyIn[i] = zeroBD;
			EvalData.stackPreflopBD[i] = zeroBD;
		...
		EvalData.lastOrbit[1] = 0;
		EvalData.lastOrbit[2] = 0;
		EvalData.lastOrbit[3] = 0;
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < PLAYERS; j++) {
				for (int k = 0; k < 4; k++) {
					...
					EvalData.raisedToBD[i][j][k] = false;
					...
					EvalData.stack$[i][j][k] = zeroBD;
					EvalData.side1$[i][j][k] = zeroBD;
					EvalData.side2$[i][j][k] = zeroBD;
					EvalData.side3$[i][j][k] = zeroBD;
					EvalData.side4$[i][j][k] = zeroBD;
					EvalData.short$[i][j][k] = zeroBD;
	...
	private static void playerBetSB() {
		...
		EvalData.playerMainPot[EvalData.seat] =EvalData. potBD;
		...
		EvalData.lastBet = 0;
		betNow =EvalData.betNowsBD;
		EvalData.bet$[0][seat][EvalData.orbit] = EvalData.BD;
	}
	...
	private static void playHandPreflop() {
		if (EvalData.playerFolded[seat])
			return;
		System.out.println("//playHandPreflop()A " + betToMe);
		action = players[seat].getDecision(betType, betToMe, betNow);
		betType = action.actionAfter;
		betNow = action.betNow;
		betToMe = action.betNow;
		System.out.println("//playHandPreflop()B " + betToMe);
	 ...
	 *******************************************************************************/
	static double run(int numGames) {
		startGame();
		return 0;
	}
