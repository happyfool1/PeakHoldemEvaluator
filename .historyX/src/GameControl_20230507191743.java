...
static void returnUncalledBet(){
	final int active = Table.PLAYERS - Table.foldCount;
		int seat = -1;
		for (int i = 0; i < Table.PLAYERS; ++i) {
			if (active == 1 && !Table.playerFolded[i]) {
				seat = i;
				break;
			}
			if (active > 1 && !Table.playerFolded[i] && Table.stgy_LastPlayerToBetOrRaise[Table.streetNumber] == i) {
				seat = i;
			}
		}
}
