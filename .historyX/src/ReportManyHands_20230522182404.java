...
public class ReportManyHands implements Constants {
	...
	static void reportBoardArrayFlopPer(int c, int r) {
		...
		for (int i = 0; i < ManyData.sumOfHoleCardValuesFlopArray.length; ++i) {
			if (ManyData.sumOfHoleCardaluesFlopArray[i] > 0) {
                table.setValueAt("Sum of hand values", row, 0);
                table.setValueAt(ManyData.sumOfHoleCardValuesFlopArray[i], row, 1);
                ++row;
            }
        }
	}
