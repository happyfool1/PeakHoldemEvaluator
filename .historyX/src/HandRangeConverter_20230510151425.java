...
public class HandRangeConverter implements Constants {
	...
	private static void copyData() {
		...
		for (int i = 0; i < HANDS; i++){
			action.rangeArray[SB][i] = rangeSBBet3CallrangeArray[i];
			action.gapsArray[SB][i] = rangeSBBet3CallgapsArray[i];
		}
		action.cards[SB] = rangeSBBet3Callcards;
		action.lowPlayabilitrhyArrayIndex[SB] = rangeSBBet3CalllowPlayabilitrhyArrayIndex;
		action.lowRangeArrayIndex [SB] = rangeSBBet3CalllowRangeArrayIndex ;
		action.lowPlayabilityArrayNumericValue[SB] = rangeSBBet3CalllowPlayabilityArrayNumericValue;
		action.gapCount[SB] = rangeSBBet3CallgapCount;
		for (int i = 0; i < HANDS; i++){
