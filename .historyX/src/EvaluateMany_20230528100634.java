	...
	private static void updateHMLArrays() {
		...
		if (EvalData.street == FLOP) {
			for (int i = 0; i < DRAW_MAX_HANDS; i++) {
				if (ManyData.hmlDraw[index][i]) {
					ManyData.hmlDraw[EvalData.seat][i]++;
				}
			}
			for (int i = 0; i < MADE_MAX_HANDS; i++) {
                if (ManyData.hmlMade[index][i]) {
                    ManyData.hmlMade[EvalData.seat][i]++;
                }
            }
		}
		...
		if (EvalData.street == TURN) {
			for (int i = 0; i < DRAW_MAX_HANDS; i++) {
                if (ManyData.hhmlDraw[index][i]) {
                    ManyData.hhmlDraw[EvalData.seat][i]++;
                }
            }
            for (int i = 0; i < MADE_MAX_HANDS; i++) {
                if (ManyData.hhmlMade[index][i]) {
                    ManyData.hhmlMade[EvalData.seat][i]++;
                }
            }
