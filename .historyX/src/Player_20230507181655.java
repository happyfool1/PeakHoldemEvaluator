...
public class Player implements Constants {
	...
	private void doFold() {
		...
		System.out.println("//doFold() seat " + this.seat + " " + EvalData.orbit);
	}
	...
	private void doLimp() {
		...
		System.out.println("//doLimp() seat " + this.seat + " moneyIn " + EvalData.moneyInBD[this.seat] + " stack " +
				EvalData.stackBD[this.seat] + " pot " +
				EvalData.potBD);
	}
	...
	private void doAllin() {
		...
		System.out.println("//doAllin() seat  " + this.seat + " bet " + bet + " call " + call + " stack "
				+ EvalData.stackBD[this.seat] + " pot " +
				EvalData.potBD);
	}
	...
	private void doAllinCall() {
		...
		System.out.println(
				"//doAllinCall() seat " + this.seat + " call " + call + " stack " + EvalData.stackBD[this.seat]
						+ " pot " +
						EvalData.potBD);
	}
	...
	private void doCalls() {
		System.out.println(
				"//doCalls()A seat " + this.seat  +" orbit " + EvalData.orbit + " betNowBD " + EvalData.betNowBD
						+ " moneyInBD " + EvalData.moneyInBD[this.seat] +  " stack "
						+ EvalData.stackBD[this.seat] + " pot " + EvalData.potBD);
		var call = EvalData.betNowBD.subtract(EvalData.moneyInBD[this.seat]);
		// Flags and counts
		EvalData.isCall[0][this.seat][EvalData.orbit] = true;
		// Money
		...
		EvalData.potBD = EvalData.potBD.add(call);
		EvalData.callBD[0][this.seat][EvalData.orbit] = call;
		//doCalls()A seat 0 orbit 1 betNowBD 8.00 moneyInBD 1 stack 199 pot 11.00
		/doCalls()B seat 0 orbit 1 betNowBD 15.00 moneyInBD 7.00 call 7.00 stack 192.00 pot 18.00
		System.out.println(
				"//doCalls()B seat " + this.seat  +" orbit " + EvalData.orbit + " betNowBD " + EvalData.betNowBD
						+ " moneyInBD " + EvalData.moneyInBD[this.seat] + " call " + call + " stack "
						+ EvalData.stackBD[this.seat] + " pot " + EvalData.potBD);
	}
	...
	private void doRaises(BigDecimal bet) {
		...
		// doRaises() seat 3 bet 6 temp 8.00 stack 192.00 pot 11.00
		// doRaises() seat 5 bet 24.00 temp 32.00 stack 168.00 pot 43.00
		System.out.println("//doRaises() seat " + this.seat + " bet " + bet + " temp " + temp
				+ " stack " + EvalData.stackBD[this.seat] + " pot " +
				EvalData.potBD);
	}
	/*- **************************************************************************** 
	 * Place SB bet
	 **************************************************************************** */
	 void playerBetSB(int seat) {
		...
		EvalData.pot$[0][this.seat][EvalData.orbit] = EvalData.potBD;
		EvalData.moneyIn$[0][this.seat][EvalData.orbit] = EvalData.moneyIn[seat];
		EvalData.lastBetBD = zeroBD;
		EvalData.betNowBD = EvalData.SBBetBD;
		EvalData.betNowBD = EvalData.SBBetBD;
		EvalData.bet$[0][this.seat][EvalData.orbit] = EvalData.betNowBD;
	 ...
	 **************************************************************************** */
	 void playerBetBB(int seat ) {
		...
		EvalData.pot$[0][this.seat][EvalData.orbit] = EvalData.potBD;
		EvalData.stackBDthis.seat[] = EvalData.stackBD[this.seat];
		EvalData.moneyInBD[this.seat] = EvalData.moneyInBD[this.seat];
		Evalthis.seatData.playerMainPotBD[seat] = EvalData.potBD;
		EvalData.pot$[0][this.seat][EvalData.orbit] = EvalData.potBD;
		EvalData.moneyIn$[0][this.seat][EvalData.orbit] = EvalData.moneyIn[this.seat];
		EvalData.betNowBD = EvalData.BBBetBD;
		EvalData.betNowBD = EvalData.BBBetBD;
		EvalData.bet$[PREFLOP][this.seat][EvalData.orbit] = EvalData.BBBetBD;
