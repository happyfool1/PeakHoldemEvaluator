	...
	*******************************************************************************/
	static void reportPreflop(int c, int r) {
		...
		if (frame3 == null) {
			...
			frame3.setTitle("Preflop step by step");
		...
		int row = 0;
		// read .txt file 
		String buffer = FileUtils.readTxtFileIntoString(EvalData.applicationDirectory + "\\HandHistory\\HHSimulation.txt");

		System.out.println("// Reading file " + buffer);
        ...
        for (int i = 0; i < 50; ++i) {
            ...
            if (parts.length == 3) {
                table3.setValueAt(parts[0], row, 0);
                table3.setValueAt(parts[1], row, 1);
                table3.setValueAt(parts[2], row, 2);
                ++row;
            }
        }
