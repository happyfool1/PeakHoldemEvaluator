...
public class GameControl implements Constants {
	...
	**************************************************************************** */
	static void preflopSimulationOneHand() {
		...
		System.out.println("//zz  " + EvalData.foldCount + " " + EvalData.potBD);
	}
	...
	*******************************************************************************/
	private static void preflopOrbit0() {
		...
		System.out.println("//seatUTG ");
		playHandPreflop();

		// Play hand MP
		seat = seatMP;
		pos = MP;
		System.out.println("//seatMP");
		playHandPreflop();

		// Play hand Cutoff
		seat = seatCO;
		pos = CO;
		System.out.println("//seatCO");
		playHandPreflop();
	...
	*******************************************************************************/
	...
	/*- 
	* This method handles uncalled bets when there is no side pot.
	* This is the simplest case, it's much more complex if there is already a side pot(s)
	* If a player has more money in that was not called, return that money.
	* 
	* In order for an uncalled bet to be returned:
	* 		There must be only 1 or 2 players still active.
	...
	* 		If 2 players are active, all others have folded, and one player has called All-in short
	* 		( not enough in stack to call or raise ) the first player will receive the uncalled patr 
	* 		of his bet back.
*/
private static void uncalledBet() {
	...
	if (!EvalData.betCalled && EvalData.betType == CHECK) {
		...
		if (active == 2 && EvalData.callAllinShortCount == 1) {
		doTheReturnCall();
		return;
	}
	if (active == 1 && EvalData.betNow != 0 || (!EvalData.betCalled && EvalData.allinCount == active)) {
		doTheReturnBet();
	}
...
static void returnUncalledBet(){
	final int active = EvalData.PLAYERS - EvalData.foldCount;
		int seat = -1;
		for (int i = 0; i < EvalData.PLAYERS; ++i) {
			if (active == 1 && !EvalData.playerFolded[i]) {
				seat = i;
				break;
			}
			if (active > 1 && !EvalData.playerFolded[i] && EvalData.stgy_LastPlayerToBetOrRaise[EvalData.streetNumber] == i) {
				seat = i;
			}
		}
		EvalData.moneyIn[seat] -= EvalData.lastBet;
		EvalData.stack[seat] += EvalData.lastBet;
		EvalData.pot -= EvalData.lastBet;
		EvalData.pot$[EvalData.streetNumber][seat][EvalData.orbit] = EvalData.pot;
		EvalData.returnedTo$[EvalData.streetNumber][seat][EvalData.orbit] = EvalData.lastBet;
		EvalData.isReturnedTo[EvalData.streetNumber][seat][EvalData.orbit] = true;
		Table.betCalled = true;
}
