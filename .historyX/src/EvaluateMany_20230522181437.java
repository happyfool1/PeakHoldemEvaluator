...
public class EvaluateMany implements Constants {
	...
	/*
	 * java texas holdem 2 hole cards and 5 board cards method to check for pairs
	 */
	...
	static void updateFlop() {
		...
		for (int i = 0; i < EvalData.flopHand.length; i++) {
			if (EvalData.flopHand[i]) {
                ++ManyData.flopHand[i];
            }
			EvalData.flopHand[i] = EvalData.both[i];
		}

		for (int i = 0; i < EvalData.turnHand.length; i++) {
			EvalData.turnHand[i] = EvalData.both[i];
		}
		...
		// TODO ++ManyData.sumOfHandValuesFlopArray[EvalData.sumOfHandValuesFlop];
		// TODO ++ManyData.sumOfBoardValuesFlopArray[EvalData.sumOfBoardValuesFlop];
		// TODO ++ManyData.sumOfHoleCardValuesArray[EvalData.sumOfHoleCardValues];

		++ManyData.hmlArray[EvalData.hmlIndexFlop];
	...
	static void calculate() {
		...
		ManyData.hmlArrayPer = calculatePercentagesArrays(ManyData.hmlArray, ManyData.flopArraysCount);
